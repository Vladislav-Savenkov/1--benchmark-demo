...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если Не ЭтоГруппа Тогда
		...
		Если Не ПустаяСтрока(ОКТМО) И (Не СтрДлина(ОКТМО) = 8 И Не СтрДлина(ОКТМО) = 11) Тогда
			УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьОбОшибке("ОКТМО должен состоять из 8 или из 11 символов.", Отказ);
		КонецЕсли;
