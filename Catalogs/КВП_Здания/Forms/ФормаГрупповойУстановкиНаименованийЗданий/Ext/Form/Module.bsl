...
Процедура ВывестиИнформациюПользователю()
	...
	УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("" + ВремяОкончанияФормирования + " - Процедура переименования зданий завершена.");
...
Процедура ЗаполнитьТабличнуюЧастьЗдания()
	...
	ОбновитьСписокПредпросмотраНаименованийЗданийНаСервере();
...
Процедура ПереименоватьЗданияПродолжение()
	...
	УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("" + ВремяНачалаФормирования + " - Запущена процедура переименования зданий.");
	
	Элементы.ТекстПрогресса.Заголовок = "Выполняется переименование зданий...";
	...
	ПодключитьОбработчикОжидания("ОбработчикПроверитьПроцессПереименования", 0.5, Истина);
	
КонецПроцедуры

&НаКлиенте
// Процедура обработчик ожидания - проверяет прогресс переименования зданий.
Процедура ОбработчикПроверитьПроцессПереименования()
	
	Прогресс = ПрочитатьПрогресс(ЭтотОбъект.ИдентификаторФоновогоЗадания);
	...
	Иначе
		...
		Если ТипЗнч(Прогресс) = Тип("Структура") И Прогресс.Свойство("Процент") Тогда
			ЭтотОбъект.ПрогрессВыполнения = Прогресс.Процент;
		КонецЕсли;
...
Функция ПрочитатьПрогресс(Знач ИдентификаторЗадания)
	...
	ПрогрессЗадания = УПЖКХ_ТиповыеМетодыСервер.ПрочитатьПрогресс(ИдентификаторЗадания);
	...
	КонецЕсли;
	ПрогрессЗадания.Вставить("ЗаданиеВыполнено", УПЖКХ_ТиповыеМетодыСервер.ЗаданиеВыполнено(ИдентификаторЗадания));
...
&НаКлиенте
// Обработчик команды "ПоказатьПредставлениеНаименований" командной панели табличного поля "Здания".
Процедура ОбновитьПредставлениеНаименований(Команда)
...
Процедура ПереименоватьЗданияНаСервере()
	...
	МассивПараметров.Добавить(Здания.Выгрузить());
	МассивПараметров.Добавить(СоставПредставленияАдреса);
	
	ФоновоеЗадание = ФоновыеЗадания.Выполнить("УПЖКХ_РаботаСОбъектамиУчетаСервер.ПереименоватьЗдания", МассивПараметров); 
	ИдентификаторФоновогоЗадания = ФоновоеЗадание.УникальныйИдентификатор;
	
КонецПроцедуры

&НаСервере
// Процедура получает для предпросмотра новые наименования выбранных зданий по настроенному шаблону.
Процедура ОбновитьСписокПредпросмотраНаименованийЗданийНаСервере()
	...
	Для Каждого ТекЗдание ИЗ Здания Цикл
		...
		ТекЗдание.НовоеНаименование = ПредставлениеНаименования; 
