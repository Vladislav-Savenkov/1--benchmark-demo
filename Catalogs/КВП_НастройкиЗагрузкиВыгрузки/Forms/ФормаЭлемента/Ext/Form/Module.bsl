...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ Отказ Тогда
		...
		Если ДеревоНастроек.Количество() > 0 Тогда
			...
			ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных = Перечисления.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных;
			...
			Для Каждого СтрокаДерева Из ДеревоНастроек Цикл
				...
				Если ЭтоЗагрузкаИзСистемСбораПоказанийПриборовУчета И НЕ КоллекцияПодстрок.Количество() = 0 Тогда
					...
					Для Каждого ТекущаяСтрока Из КоллекцияПодстрок Цикл
						НоваяСтрока = ТекущийОбъект.НастройкиСоответствияПолей.Добавить();
						ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока);
					КонецЦикла;
					
				ИначеЕсли ЭтоУниверсальнаяВыгрузкаДанных
					И СтрокаДерева.Параметр = ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги Тогда
					
					Если СтрокаДерева.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоСчетчикам Тогда
						
						Сч = 1;
						
						Для Каждого ТекущаяСтрока Из БлокСведенийПоСчетчикам Цикл
							
							НоваяСтрока = ТекущийОбъект.НастройкиСоответствияПолей.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока);
							
							НоваяСтрока.Параметр                  = ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги;
							НоваяСтрока.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.Счетчики;
					...
					ИначеЕсли СтрокаДерева.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоУслугам Тогда
						
						Сч = 1;
						
						Для Каждого ТекущаяСтрока Из БлокСведенийПоУслугам Цикл
							
							НоваяСтрока = ТекущийОбъект.НастройкиСоответствияПолей.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекущаяСтрока);
							
							НоваяСтрока.Параметр                  = ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги;
							НоваяСтрока.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.Услуги;
							НоваяСтрока.НомерКолонки              = Сч;
...
&НаКлиенте
Процедура РазделительКолонокПриИзменении(Элемент)
	
	МассивЭлементов = Новый Массив;
	
	МассивЭлементов.Добавить(Элементы.ДеревоНастройкаСоответствияПолейРазделительПослеЗначения);
	МассивЭлементов.Добавить(Элементы.БлокСведенийПоСчетчикамРазделительПослеЗначения);
	МассивЭлементов.Добавить(Элементы.БлокСведенийПоУслугамРазделительПослеЗначения);
...
&НаКлиенте
Процедура ДеревоНастройкаСоответствияПолейПередНачаломДобавления(Элемент, Отказ, Копирование, Родитель, Группа, Параметр)
	
	Если Копирование Тогда
		
		Если ЭтоУниверсальнаяВыгрузкаДанных Тогда
			
			НоваяСтрока = ДеревоНастройкаСоответствияПолей.ПолучитьЭлементы().Добавить();
			
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Элементы.ДеревоНастройкаСоответствияПолей.ТекущиеДанные);
			
			Если НоваяСтрока.Параметр = ПредопределенноеЗначение("Перечисление.УПЖКХ_ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги") Тогда
				
				НоваяСтрока.ТипПараметраБлокаСведений = ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ПустаяСсылка");
				
				Если ЭтоTXT ИЛИ ЭтоCSV ИЛИ ЭтоXLS Тогда
					НоваяСтрока.НомерКолонки = ПолучитьНомерКолонки();
				КонецЕсли;
				
				ИдентификаторСтроки = НоваяСтрока.ПолучитьИдентификатор();
				
				ДеревоНастройкаСоответствияПолейПараметрОбработкаВыбораУниверсальнаяВыгрузкаДанных(ИдентификаторСтроки, Отказ);
				
				Если Отказ Тогда
					ДеревоНастройкаСоответствияПолей.ПолучитьЭлементы().Удалить(ДеревоНастройкаСоответствияПолей.НайтиПоИдентификатору(ИдентификаторСтроки));
				КонецЕсли;
	...
	Если НЕ ТекущиеДанные = Неопределено
	   И НЕ ТекущиеДанные.Параметр = ВыбранноеЗначение Тогда
		...
		Если ЭтоУниверсальнаяВыгрузкаДанных
		   И ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.УПЖКХ_ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги") Тогда
			
			ДеревоНастройкаСоответствияПолейПараметрОбработкаВыбораУниверсальнаяВыгрузкаДанных(Элементы.ДеревоНастройкаСоответствияПолей.ТекущаяСтрока);
...
Процедура УдалитьПробелыВСтрокеДляИмениПоляDBF(Элемент)
	...
	Если Элемент.Имя = "ДеревоНастройкаСоответствияПолейИмяПоляDBF" Тогда
		ТекущиеДанные = Элементы.ДеревоНастройкаСоответствияПолей.ТекущиеДанные;
	ИначеЕсли Элемент.Имя = "БлокСведенийПоСчетчикамИмяПоляDBF" Тогда
		ТекущиеДанные = Элементы.БлокСведенийПоСчетчикам.ТекущиеДанные;
	ИначеЕсли Элемент.Имя = "БлокСведенийПоУслугамИмяПоляDBF" Тогда
		ТекущиеДанные = Элементы.БлокСведенийПоУслугам.ТекущиеДанные;
	КонецЕсли;
...
&НаКлиенте
Процедура ДеревоНастройкаСоответствияПолейТипПараметраБлокаСведенийПриИзменении(Элемент)
	...
	Для Каждого ТекущаяСтрока Из КоллекцияСтрок Цикл
		Если ТекущаяСтрока.Параметр = ПредопределенноеЗначение("Перечисление.УПЖКХ_ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги")
		   И ТекущаяСтрока.ТипПараметраБлокаСведений = ТекущиеДанные.ТипПараметраБлокаСведений Тогда
	...
	Если КоличествоПараметров > 1 Тогда
		...
		ТекущиеДанные.ТипПараметраБлокаСведений = ?(ТекущиеДанные.ТипПараметраБлокаСведений = ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоСчетчикам"),
			ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоУслугам"),
			ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоСчетчикам"));
		
		Если ТекущиеДанные.ТипПараметраБлокаСведений = ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоСчетчикам") Тогда
			Текст = "счетчикам";
		Иначе
			Текст = "услугам";
		КонецЕсли;
		
		УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьОбОшибке("В таблице уже имеется параметр ""Счетчики и услуги"" по " + Текст + ".");
		
	Иначе
		
		ИзменитьТекущуюСтраницуНастроекБлокаСведений(ТекущиеДанные.ТипПараметраБлокаСведений);
		
		ОбновитьЗаголовокГруппыНастройкиПараметра(Строка(ТекущиеДанные.Параметр), ТекущиеДанные.ТипПараметраБлокаСведений);
...
&НаКлиенте
Процедура ДеревоНастройкаСоответствияПолейПараметрОбработкаВыбораУниверсальнаяВыгрузкаДанных(НомерСтроки, Отказ = Ложь)
	
	ТекущиеДанные = ДеревоНастройкаСоответствияПолей.НайтиПоИдентификатору(НомерСтроки);
	
	Если ТекущиеДанные = Неопределено Тогда
		Возврат;
	КонецЕсли;
	
	ТипОсновнойПоСчетчикам = ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоСчетчикам");
	ТипОсновнойПоУслугам   = ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоУслугам");
	ТипПустаяСсылка        = ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ПустаяСсылка");
	
	КоллекцияСтрок                  = ПолучитьКоллекциюСтрокДереваНаКлиенте();
	КоличествоПараметровПоСчетчикам = 0;
	КоличествоПараметровПоУслугам   = 0;
	
	Для Каждого ТекущаяСтрока Из КоллекцияСтрок Цикл
		Если ТекущаяСтрока.Параметр = ПредопределенноеЗначение("Перечисление.УПЖКХ_ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги") Тогда
			Если ТекущаяСтрока.ТипПараметраБлокаСведений = ТипОсновнойПоСчетчикам Тогда
				КоличествоПараметровПоСчетчикам = КоличествоПараметровПоСчетчикам + 1;
			ИначеЕсли ТекущаяСтрока.ТипПараметраБлокаСведений = ТипОсновнойПоУслугам Тогда
				КоличествоПараметровПоУслугам = КоличествоПараметровПоУслугам + 1;
			КонецЕсли;
		КонецЕсли;
	КонецЦикла;
	
	ТипПараметраБлокаСведений = ТипПустаяСсылка;
	
	Если КоличествоПараметровПоСчетчикам = 0 Тогда
		ТипПараметраБлокаСведений = ТипОсновнойПоСчетчикам;
	ИначеЕсли КоличествоПараметровПоУслугам = 0 Тогда
		ТипПараметраБлокаСведений = ТипОсновнойПоУслугам;
	КонецЕсли;
	
	Если ТипПараметраБлокаСведений = ТипПустаяСсылка Тогда
		ТекущиеДанные.Параметр = ПредопределенноеЗначение("Перечисление.УПЖКХ_ПараметрыУниверсальнойВыгрузкиДанных.ПустаяСсылка");
		УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьОбОшибке("В таблице параметров уже имеется параметр ""Счетчики и услуги"" для выгрузки сведений по счетчикам и услугам.", Отказ);
	Иначе
		ОчиститьНастройкиПередИзменениемПараметраНаСервере(НомерСтроки);
		
		ТекущиеДанные.Параметр                  = ПредопределенноеЗначение("Перечисление.УПЖКХ_ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги");
		ТекущиеДанные.ТипПараметраБлокаСведений = ТипПараметраБлокаСведений;
...
Процедура ПроверитьНастройкиВДереве(Отказ)
				...
				Если ТекущаяСтрока.СпособОпределенияПУ = СтруктураЗначенийПолей.СпСпособыОпределенияПУ[1].Значение // "по услуге"
				   И ТекущаяСтрока.СпособОпределенияУслуги = СтруктураЗначенийПолей.СпСпособыОпределенияУслуги[1].Значение Тогда // "по ссылке на услугу"
					Если ПустойНомерКолонкиЛицевогоСчета Тогда
						УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не указан номер колонки Лицевого счета, так как прибор учета "
																			+ ТекущаяСтрока.ИмяПараметра + """ определяется по ссылке на услугу.",,,,Отказ);
					КонецЕсли;
					Если ТекущаяСтрока.Услуга = Справочники.КВП_Услуги.ПустаяСсылка() Тогда
						УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не указана услуга из справочника ""Услуги"" прибора учета """
																			+ ТекущаяСтрока.ИмяПараметра + """.",,,,Отказ);
					КонецЕсли;
				КонецЕсли;
	...
	ИначеЕсли ЭтоУниверсальнаяВыгрузкаДанных Тогда
		...
		// Наличие параметра "Лицевой счет" или "Адрес":
		
		НайденнаяСтрокаЛицевойСчет = КоллекцияСтрокДерева.Найти(ПараметрыУниверсальнойВыгрузкиДанных.ЛицевойСчет, "Параметр");
		НайденнаяСтрокаАдрес       = КоллекцияСтрокДерева.Найти(ПараметрыУниверсальнойВыгрузкиДанных.Адрес, "Параметр");
		Если НайденнаяСтрокаЛицевойСчет = Неопределено И НайденнаяСтрокаАдрес = Неопределено Тогда
			УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("В таблице параметров выгрузки должна быть хотя бы одна строка "
																+ "с параметром ""Лицевой счет"" или ""Адрес"".",,
																"ДеревоНастройкаСоответствияПолей",, Отказ);
		КонецЕсли;
		
		СчПустыхНомеровКолонок = 0;
		
		Если ЭтоDBF Тогда
			мИменаDBF = КоллекцияСтрокДерева.ВыгрузитьКолонку("ИмяПоляDBF");
			
			Отбор = Новый Структура("Параметр, ТипПараметраБлокаСведений", ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги);
			
			Отбор.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.ОсновнойПоСчетчикам;
			ЕстьПараметрыСчетчиков = Ложь;
			Если КоллекцияСтрокДерева.НайтиСтроки(Отбор).Количество() > 0 Тогда
				Для Каждого ТекущаяСтрокаСчетчики Из БлокСведенийПоСчетчикам Цикл
					мИменаDBF.Добавить(ТекущаяСтрокаСчетчики.ИмяПоляDBF);
					ЕстьПараметрыСчетчиков = Истина;
				КонецЦикла;
			КонецЕсли;
			
			Отбор.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.ОсновнойПоУслугам;
			ЕстьПараметрыУслуг = Ложь;
			Если КоллекцияСтрокДерева.НайтиСтроки(Отбор).Количество() > 0 Тогда
				Для Каждого ТекущаяСтрокаУслуги Из БлокСведенийПоУслугам Цикл
					мИменаDBF.Добавить(ТекущаяСтрокаУслуги.ИмяПоляDBF);
					ЕстьПараметрыУслуг = Истина;
				КонецЦикла;
			КонецЕсли;
			
			СтрокаИменDBF = ";" + СтрСоединить(мИменаDBF, ";") + ";";
			ТекстСообщения = "";
			Для Каждого ТекущееИмяDBF Из мИменаDBF Цикл
				Если СтрЧислоВхождений(СтрокаИменDBF, ";" + ТекущееИмяDBF + ";") > 1 Тогда
					Если ЕстьПараметрыСчетчиков И ЕстьПараметрыУслуг Тогда
						ТекстСообщения = "Имена полей DBF не должны совпадать во всех таблицах с параметрами выгрузки.";
					ИначеЕсли ЕстьПараметрыСчетчиков И НЕ ЕстьПараметрыУслуг Тогда
						ТекстСообщения = "Имена полей DBF не должны совпадать в таблице основных параметров и параметров сведений по счетчикам.";
					ИначеЕсли НЕ ЕстьПараметрыСчетчиков И ЕстьПараметрыУслуг Тогда
						ТекстСообщения = "Имена полей DBF не должны совпадать в таблице основных параметров и параметров сведений по услугам.";
					Иначе
						ТекстСообщения = "Имена полей DBF не должны совпадать в таблице основных параметров.";
					КонецЕсли;
					Прервать;
				КонецЕсли;
			КонецЦикла;
			...
			Если НЕ ТекстСообщения = "" Тогда
				УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю(ТекстСообщения,,,, Отказ);
			КонецЕсли;
		...
		Для Каждого ТекущаяСтрока Из КоллекцияСтрокДерева Цикл
			...
			Если ТекущаяСтрока.НомерКолонки = 0 Тогда
				СчПустыхНомеровКолонок = СчПустыхНомеровКолонок + 1;
			КонецЕсли;
			
			Если НЕ ЗначениеЗаполнено(ТекущаяСтрока.Параметр) Тогда
				УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не указан параметр в таблице состава файла.",,"ДеревоНастройкаСоответствияПолей",, Отказ);
			КонецЕсли;
			
			Если ТекущаяСтрока.Параметр = ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги Тогда
				
				Если ТекущаяСтрока.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.ОсновнойПоСчетчикам Тогда
					
					Если БлокСведенийПоСчетчикам.Количество() = 0 Тогда
						УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не указаны параметры в таблице состава сведений по счетчикам.",,
																			 "БлокСведенийПоСчетчикам",, Отказ);
					Иначе
						Для Каждого ТекущаяСтрокаСведений Из БлокСведенийПоСчетчикам Цикл
							Если НЕ ЗначениеЗаполнено(ТекущаяСтрокаСведений.ПараметрБлокаСведений) Тогда
								УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не задан параметр в таблице состава сведений по счетчикам.",,
																					 "БлокСведенийПоСчетчикам",, Отказ);
							КонецЕсли;
							Если ЭтоDBF И ПустаяСтрока(ТекущаяСтрокаСведений.ИмяПоляDBF) Тогда
								УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не задано имя поля для файла DBF параметра """
																					+ ТекущаяСтрокаСведений.ПараметрБлокаСведений
																					+ """ в таблице состава сведений по счетчикам.",,
																					"БлокСведенийПоСчетчикам",, Отказ);
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
					
				ИначеЕсли ТекущаяСтрока.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.ОсновнойПоУслугам Тогда
					
					Если БлокСведенийПоУслугам.Количество() = 0 Тогда
						УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не указаны параметры в таблице состава сведений по услугам.",,
																			 "БлокСведенийПоУслугам",, Отказ);
					Иначе
						Для Каждого ТекущаяСтрокаСведений Из БлокСведенийПоУслугам Цикл
							Если НЕ ЗначениеЗаполнено(ТекущаяСтрокаСведений.ПараметрБлокаСведений) Тогда
								УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не задан параметр в таблице состава сведений по услугам.",,
																					 "БлокСведенийПоУслугам",, Отказ);
							КонецЕсли;
							Если ЭтоDBF И ПустаяСтрока(ТекущаяСтрокаСведений.ИмяПоляDBF) Тогда
								УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не задано имя поля для файла DBF параметра """
																					+ ТекущаяСтрокаСведений.ПараметрБлокаСведений
																					+ """ в таблице состава сведений по услугам.",,
																					"БлокСведенийПоУслугам",, Отказ);
							КонецЕсли;
						КонецЦикла;
					КонецЕсли;
			...
			ИначеЕсли ТекущаяСтрока.Параметр = ПараметрыУниверсальнойВыгрузкиДанных.КБК Тогда
				
				Если ТекущаяСтрока.НалогКБК.Пустая() Тогда
					УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не задано значение поля Налог для параметра ""КБК""",,"Налог",, Отказ);
				КонецЕсли;
				
			Иначе
				
				Если ЭтоDBF И ПустаяСтрока(ТекущаяСтрока.ИмяПоляDBF) Тогда
					УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не задано имя поля для файла DBF параметра """
																		+ ТекущаяСтрока.Параметр +  """.",,"ДеревоНастройкаСоответствияПолей",, Отказ);
				КонецЕсли;
		...
		Если НЕ СчПустыхНомеровКолонок = 0 И НЕ ЭтоDBF Тогда
			УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("В таблице параметров для всех строк необходимо указать номера колонок.",,"ДеревоНастройкаСоответствияПолей",, Отказ);
		КонецЕсли;
...
Процедура УправлениеФормой()
	...
	Элементы.ДеревоНастройкаСоответствияПолейРазделительПослеЗначения.Видимость = ЭтоУниверсальнаяВыгрузкаДанных;
	...
	ИначеЕсли ЭтоУниверсальнаяВыгрузкаДанных Тогда
		...
		Элементы.БлокСведенийПоСчетчикамИмяПоляDBF.Видимость = ЭтоDBF;
		Элементы.БлокСведенийПоУслугамИмяПоляDBF.Видимость   = ЭтоDBF;
		
		Элементы.ДеревоНастройкаСоответствияПолейНомерКолонки.Видимость = НЕ ЭтоDBF;
		
		Элементы.ДеревоНастройкаСоответствияПолейРазделительПослеЗначения.Видимость = ЭтоTXT;
		
		Элементы.БлокСведенийПоСчетчикамРазделительПослеЗначения.Видимость = ЭтоTXT;
		Элементы.БлокСведенийПоУслугамРазделительПослеЗначения.Видимость   = ЭтоTXT;
...
Процедура ЗагрузитьНастройкиСоответствияПолейВДерево()
	...
	ИначеЕсли ЭтоУниверсальнаяВыгрузкаДанных Тогда
		...
		ТипыПараметровБлоковСведений = Перечисления.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных;
		НастройкиСоответствияПолей   = РеквизитФормыВЗначение("Объект").НастройкиСоответствияПолей.Выгрузить();
		
		Для Каждого ТекущаяСтрока Из НастройкиСоответствияПолей Цикл
			
			Если НЕ ТекущаяСтрока.Параметр = ПараметрыУВД.СчетчикиИУслуги
			 ИЛИ (ТекущаяСтрока.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.ОсновнойПоСчетчикам
				  ИЛИ ТекущаяСтрока.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.ОсновнойПоУслугам) Тогда
		...
		СтрокиСчетчикиИУслуги = НастройкиСоответствияПолей.НайтиСтроки(Новый Структура("Параметр", ПараметрыУВД.СчетчикиИУслуги));
		ИмеютсяСтрокиБлоков   = СтрокиСчетчикиИУслуги.Количество() > 0;
		
		Если ИмеютсяСтрокиБлоков Тогда
			
			НастройкиБлоков = НастройкиСоответствияПолей.Скопировать(СтрокиСчетчикиИУслуги);
			Отбор           = Новый Структура("ТипПараметраБлокаСведений");
			
			// Блок "Счетчики":
			
			Отбор.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.Счетчики;
			СтрокиСчетчики = НастройкиБлоков.НайтиСтроки(Отбор);
			
			Для Каждого ТекущаяСтрокаСчетчики Из СтрокиСчетчики Цикл
				
				НоваяСтрокаСчетчики = БлокСведенийПоСчетчикам.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаСчетчики, ТекущаяСтрокаСчетчики);
				
			КонецЦикла;
			
			// Блок "Услуги":
			
			Отбор.ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.Услуги;
			СтрокиУслуги = НастройкиБлоков.НайтиСтроки(Отбор);
			
			Для Каждого ТекущаяСтрокаУслуги Из СтрокиУслуги Цикл
				
				НоваяСтрокаУслуги = БлокСведенийПоУслугам.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаУслуги, ТекущаяСтрокаУслуги);
	...
	Если НЕ ТекущиеДанныеФормы = Неопределено Тогда
		...
		Для Каждого ТекущаяКолонка Из КолонкиТаблицы Цикл
			...
			Если НЕ ИмяКолонки = "Параметр"
			   ...
			   И НЕ СтрНачинаетсяС(ИмяКолонки, "Удалить") Тогда
...
Процедура ЗаполнитьСпискиВыбораПолейФормы()
	...
	Для Каждого ТекЭлемент Из СтруктураЗначенийПолей.СпРазделителейПослеЗначения Цикл
		Элементы.ДеревоНастройкаСоответствияПолейРазделительПослеЗначения.СписокВыбора.Добавить(ТекЭлемент.Значение, ТекЭлемент.Представление);
		Элементы.БлокСведенийПоСчетчикамРазделительПослеЗначения.СписокВыбора.Добавить(ТекЭлемент.Значение, ТекЭлемент.Представление);
		Элементы.БлокСведенийПоУслугамРазделительПослеЗначения.СписокВыбора.Добавить(ТекЭлемент.Значение, ТекЭлемент.Представление);
	КонецЦикла;
	...
	СписокПараметровБлокаСведенийПоСчетчикам = Перечисления.УПЖКХ_ПараметрыБлоковСведенийУниверсальнойВыгрузкиДанных.СписокПараметровБлокаСведений("Счетчики");
	Для Каждого ТекЭлемент Из СписокПараметровБлокаСведенийПоСчетчикам Цикл
		Элементы.БлокСведенийПоСчетчикамПараметрБлокаСведений.СписокВыбора.Добавить(ТекЭлемент.Значение, ТекЭлемент.Представление);
	КонецЦикла;
	
	СписокПараметровБлокаСведенийПоУслугам = Перечисления.УПЖКХ_ПараметрыБлоковСведенийУниверсальнойВыгрузкиДанных.СписокПараметровБлокаСведений("Услуги");
	Для Каждого ТекЭлемент Из СписокПараметровБлокаСведенийПоУслугам Цикл
		Элементы.БлокСведенийПоУслугамПараметрБлокаСведений.СписокВыбора.Добавить(ТекЭлемент.Значение, ТекЭлемент.Представление);
	КонецЦикла;
...
Процедура ОтобразитьСтраницуНастроекПараметра()
	...
	ИначеЕсли ЭтоУниверсальнаяВыгрузкаДанных Тогда
		...
		СоответствияСтраницыНастроекТекущемуПараметру.Вставить("Счетчики и услуги",                      Элементы.СчетчикиИУслуги);
	...
	Иначе
		...
		ОбновитьЗаголовокГруппыНастройкиПараметра(ПараметрСтрокой, ТекущиеДанные.ТипПараметраБлокаСведений);
...
&НаКлиенте
Процедура ОбновитьЗаголовокГруппыНастройкиПараметра(ПараметрСтрокой, ТипПараметраБлокаСведений)
	
	Если ЭтоУниверсальнаяВыгрузкаДанных И ПараметрСтрокой = "Счетчики и услуги" Тогда
		
		Если ТипПараметраБлокаСведений = ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоСчетчикам") Тогда
			
			Элементы.ГруппаНастройкиПараметра.Заголовок = "Настройки параметра """ + ПараметрСтрокой + """ - Сведения по счетчикам:";
			
		ИначеЕсли ТипПараметраБлокаСведений = ПредопределенноеЗначение("Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ОсновнойПоУслугам") Тогда
			
			Элементы.ГруппаНастройкиПараметра.Заголовок = "Настройки параметра """ + ПараметрСтрокой + """ - Сведения по услугам:";
...
Процедура ОтобразитьНастройкиНаСтраницеНастроекПараметра(ТекущиеДанные)
	...
	ИначеЕсли ЭтоУниверсальнаяВыгрузкаДанных Тогда
		...
		Элементы.ДеревоНастройкаСоответствияПолейГруппаDBF.Доступность = НЕ Параметр = ПредопределенноеЗначение("Перечисление.УПЖКХ_ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги");
		
		Элементы.ДеревоНастройкаСоответствияПолейРазделительПослеЗначения.Доступность = НЕ Параметр = ПредопределенноеЗначение("Перечисление.УПЖКХ_ПараметрыУниверсальнойВыгрузкиДанных.СчетчикиИУслуги");
...
&НаСервере
Процедура ИзменитьТекущуюСтраницуНастроекБлокаСведений(ТипПараметраБлокаСведений)
	
	ТипыПараметровБлоковСведений = Перечисления.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных;
	
	Если ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.ОсновнойПоСчетчикам Тогда
		
		Элементы.СтраницыБлоковСведений.ТекущаяСтраница = Элементы.СтраницаБлокСведенийПоСчетчикам;
		
	ИначеЕсли ТипПараметраБлокаСведений = ТипыПараметровБлоковСведений.ОсновнойПоУслугам Тогда
		
		Элементы.СтраницыБлоковСведений.ТекущаяСтраница = Элементы.СтраницаБлокСведенийПоУслугам;
		
	Иначе
		
		Элементы.СчетчикиИУслуги.Видимость = Ложь;
