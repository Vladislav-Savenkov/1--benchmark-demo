...
Процедура УправлениеФормой(Форма)
	...
	ВидимостьПодсказкиИспользованиеНормы = Ложь;
	Если Объект.Основание = ПредопределенноеЗначение("Перечисление.КВП_ОснованияНормПотребления.ОдинЧеловек")
		 И Объект.ВидЖильцов = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыЖильцов.БлизкиеРодственники") Тогда
		ВидимостьПодсказкиИспользованиеНормы = Истина;
	ИначеЕсли Объект.ВариантНастройки = ПредопределенноеЗначение("Перечисление.КВП_ВариантыНастроекРасчетаНормативовПотребления.Расширенный") Тогда
		СтрокиБлизкиеРодственники = Объект.Характеристики.НайтиСтроки(Новый Структура("Вид", "Близких родственников"));
		Если СтрокиБлизкиеРодственники.Количество() > 0 Тогда
			ВидимостьПодсказкиИспользованиеНормы = Истина;
		КонецЕсли;
	КонецЕсли;
	
	Элементы.ГруппаПодсказкаИспользованиеНормы.Видимость = ВидимостьПодсказкиИспользованиеНормы;
...
Процедура ТипОбъектаНазначенияПриИзменении(Элемент)
	...
	Если Не Объект.НормаНаЛицевойСчет
	  ...
	  И Не Объект.Основание = ПредопределенноеЗначение("Перечисление.КВП_ОснованияНормПотребления.Площадь")
	  И Не Объект.Основание = ПредопределенноеЗначение("Перечисление.КВП_ОснованияНормПотребления.ОдинЧеловек") Тогда
		Объект.Основание = ПредопределенноеЗначение("Перечисление.КВП_ОснованияНормПотребления.ПустаяСсылка");
	КонецЕсли;
	
	Если Не Объект.НормаНаЛицевойСчет
		 И (Объект.ВидЖильцов = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыЖильцов.БлизкиеРодственники")
		 Или Объект.ВидЖильцов = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыЖильцов.Льготники")) Тогда
		врВидЖильцов = "";
		Объект.ВидЖильцов = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыЖильцов.ПустаяСсылка");
	КонецЕсли;
...
&НаКлиенте
Процедура врВидЖильцовПриИзменении(Элемент)
	УправлениеФормой(ЭтаФорма);
КонецПроцедуры
