...
Процедура УстановитьЗаголовкиФормул(Форма)
	...
	// Описание формул по постановлениям.
	Элементы.НадписьОписаниеПостановление307.Видимость = (Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.Постановление307"));
	Элементы.НадписьОписаниеПостановление354.Видимость = (Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.Постановление354"));
	Элементы.НадписьОписаниеПостановление344.Видимость = (Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.Постановление344"));
	Элементы.ОписаниеПостановление1708.Видимость       = (Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.Постановление1708"));
...
Процедура УстановитьВидимостьИДоступностьРеквизитовФормы()
	...
	Если Объект.СпособРасчета = ПредопределенноеЗначение("Перечисление.КВП_СпособыРасчета.ПоПоказаниямСчетчика")
	 ИЛИ Объект.СпособРасчета = ПредопределенноеЗначение("Перечисление.КВП_СпособыРасчета.ПоПоказаниямСчетчикаИНорме") Тогда
		...
		// Варианты начисления доступны только для услуг по показаниям счетчика.
		Элементы.ВариантыНачисленияУслуги.Видимость = Истина;
	...
	Иначе
		...
		Элементы.ВариантыНачисленияУслуги.Видимость = Ложь;
	КонецЕсли;
	...
	Если Объект.СпособРасчета = ПредопределенноеЗначение("Перечисление.КВП_СпособыРасчета.ПоПоказаниямСчетчика")
	 ИЛИ Объект.СпособРасчета = ПредопределенноеЗначение("Перечисление.КВП_СпособыРасчета.ПоПоказаниямСчетчикаИНорме") Тогда
		...
		Для Каждого ТекРеквизит Из мТаблицаРеквизитовНастроекИзМакета Цикл
			Элементы[ТекРеквизит.ИмяРеквизита].Видимость = (Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.РучнаяНастройка"));
		КонецЦикла;
...
Процедура ЗаполнитьРеквизитыФормы()
	...
	Если Не Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.РучнаяНастройка") Тогда
		...
		Для Каждого ТекСтрока Из мТаблицаНастроекПостановленияИзМакета Цикл
			...
			Для Каждого ТекРеквизит Из НайденныеРеквизиты Цикл
				Если ТекРеквизит.Признак И Не ТекСтрока.Постановление = Строка(Объект.ВариантыНачисленияУслуги) Тогда
					Объект[ТекРеквизит.ИмяРеквизита] = Ложь;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла;
		...
		НайденныеНастройки = мТаблицаНастроекПостановленияИзМакета.НайтиСтроки(Новый Структура("Постановление", ПолучитьИмяЗначенияПеречисленияВариантыНачисленияУслуги(Строка(Объект.ВариантыНачисленияУслуги))));
	...
	Если Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.Постановление307")
																									Или Объект.ИспользоватьФормулу9 Тогда
	...
	ИначеЕсли Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.Постановление354")
				Или Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.Постановление344") Тогда
...
Процедура УстановитьЗначенияРеквизитовПоУмолчанию()
	...
	Объект.ВариантыНачисленияУслуги                     = Перечисления.УПЖКХ_ВариантыНачисленияУслуги.РучнаяНастройка;
...
&НаСервере
// Возвращает наименование значения перечисления по синониму значения.
Функция ПолучитьИмяЗначенияПеречисленияВариантыНачисленияУслуги(СинонимЗначения)
	
	КолекцияЗначенийПеречисления = Метаданные.Перечисления.УПЖКХ_ВариантыНачисленияУслуги.ЗначенияПеречисления;
	Для каждого ТекЗначение из КолекцияЗначенийПеречисления Цикл
		Если ТекЗначение.Синоним = СинонимЗначения Тогда
...
&НаКлиенте
// Обработчик события "ОбработкаВыбора" поля выбора варианта начисления услуги.
Процедура ВариантыНачисленияУслугиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	
	// При выборе любого постановления если установлены настройки формул - очищаем их.
	Если Объект.ВариантыНачисленияУслуги = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.РучнаяНастройка")
	   И Не Объект.ВариантыНачисленияУслуги = ВыбранноеЗначение Тогда
		...
		Если ЗадаватьВопрос Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("ВариантыНачисленияУслугиОбработкаВыбораЗавершение", ЭтотОбъект, ДопПараметры);
			ТекстВопроса       = "Настройки отдельных формул будут очищены. Продолжить?";
	...
	ВариантыНачисленияУслугиОбработкаВыбораФрагмент(ВыбранноеЗначение);
	
КонецПроцедуры // ВариантыНачисленияУслугиОбработкаВыбора()

&НаКлиенте
// Процедура-обработка резульрата вопроса, вызванной в процедуре "ВариантыНачисленияУслугиОбработкаВыбора()".
//
// Параметры:
//  РезультатВопроса		 - 	КодВозвратаДиалога - код ответа.
//  ДополнительныеПараметры	 - 	Структура.
//
Процедура ВариантыНачисленияУслугиОбработкаВыбораЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	ВыбранноеЗначение = ДополнительныеПараметры.ВыбранноеЗначение;
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		
		Объект.ВариантыНачисленияУслуги = ВыбранноеЗначение;
...
КонецПроцедуры // ВариантыНачисленияУслугиОбработкаВыбораЗавершение()

&НаКлиенте
// Процедура устанавливает значения реквизитов объекта.
Процедура ВариантыНачисленияУслугиОбработкаВыбораФрагмент(Знач ВыбранноеЗначение)
	
	Если Не ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВариантыНачисленияУслуги.РучнаяНастройка") Тогда
		
		ВыбранноеЗначение = ПолучитьИмяЗначенияПеречисленияВариантыНачисленияУслуги(Строка(ВыбранноеЗначение));
...
Процедура ВариантыНачисленияУслугиПриИзменении(Элемент)
