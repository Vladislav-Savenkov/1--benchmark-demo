...
Процедура ПолучитьБлизкихРодственников(ТолькоБлизкиеРодственники = Ложь)
	...
	|ГДЕ
	|	НЕ УПЖКХ_СтепеньРодстваФизЛицСрезПоследних.СтепеньРодства = ЗНАЧЕНИЕ(Справочник.КВП_ТипыРодственныхСвязей.ПустаяСсылка)
	|	И &ТекстУсловияБлизкоеРодство";
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	| АдресПрибытия",
	Объект.Ссылка,
	КонтактнаяИнформацияПолеУПЖКХ_АдресВыбытияЖильца,
	КонтактнаяИнформацияПолеУПЖКХ_АдресПрибытияЖильца);
	
	Оповестить("ЗаписанЭлементСправочникаУПЖКХ_Жильцы", СтруктураПараметров);
