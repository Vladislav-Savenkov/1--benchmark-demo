...
Функция НомерВерсииТиповойКонфигурации(ВерсияЖКХ) Экспорт
	...
	Если ВерсияЖКХ = "3.0.73.2" Тогда
		Возврат "3.0.73.50";
	КонецЕсли;
	Если ВерсияЖКХ = "3.0.73.1" Тогда
		Возврат "3.0.73.38";
	КонецЕсли;
	Если ВерсияЖКХ = "3.0.72.7"
	 ИЛИ ВерсияЖКХ = "3.0.72.6" Тогда
		Возврат "3.0.72.72";
	КонецЕсли;
	Если ВерсияЖКХ = "3.0.72.5" Тогда
		Возврат "3.0.72.70";
	КонецЕсли;
	Если ВерсияЖКХ = "3.0.72.4"
	 ИЛИ ВерсияЖКХ = "3.0.72.3" Тогда
		Возврат "3.0.72.66";
	КонецЕсли;
	Если ВерсияЖКХ = "3.0.72.2" Тогда
		Возврат "3.0.72.60";
	КонецЕсли;
...
Функция ВерсияЯвляетсяНеофициальной(НомерВерсии) Экспорт
	...
	Если НомерВерсии = "3.0.72.7"
	 ИЛИ НомерВерсии = "3.0.71.9"
...
Функция ОписаниеВерсийКритичныхОбновлений()
	...
	НоваяВерсия = ТаблицаКритичныхВерсий.Добавить();
	НоваяВерсия.Версия   = "3.0.72.7";
	НоваяВерсия.Описание = "В данном обновлении будут выполнены важные обработчики обновления данных!";
...
Функция ОпределитьКритичностьОбновления(СтараяВерсия, НоваяВерсия) Экспорт
	...
	Если СтруктураКритичности.Критичность Тогда
		...
		Если СтруктураКритичности.Версия = "3.0.72.7" Тогда
			
			// Если переход с 2.0 ранее не выполнялся, тогда процедуры доп. обработки данных
			// запускаться не будут, необходимости в определении версии как критичной нет.
