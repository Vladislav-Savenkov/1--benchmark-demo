...
Функция ПолучитьНастройкиИнтеграцииСГолосовымиСервисами() Экспорт
	...
	Если ТипЗнч(СтруктураНастроек) = Тип("Структура") Тогда
		...
		КонецЕсли;
		Если НЕ СтруктураНастроек.Свойство("ВыборУслуг") Тогда
			СтруктураНастроек.Вставить("ВыборУслуг", "Все услуги");
		КонецЕсли;
		Если НЕ СтруктураНастроек.Свойство("ВыбранныеУслуги") Тогда
			СтруктураНастроек.Вставить("ВыбранныеУслуги", Новый СписокЗначений);
		КонецЕсли;
	Иначе
...
Функция ПодготовитьОбщуюСтруктуруДанныхДляВыгрузки(СведенияОПодписке, МассивЛС = Неопределено, ДатаВыгрузки = Неопределено) Экспорт
	...
	Если ВыгружатьСведенияОСчетчиках Тогда
		...
		|				И &УсловиеОтбораПоУслуге) КАК КВП_ЗакрепленныеСчетчикиСрезПоследних
		|ГДЕ
		...
		ВыборУслуг      = НастройкиОбменаСГолосовымиСервисами.ВыборУслуг;
		ВыбранныеУслуги = НастройкиОбменаСГолосовымиСервисами.ВыбранныеУслуги;
		
		Запрос.УстановитьПараметр("ВыбранныеУслуги", ВыбранныеУслуги);
		
		Если ВыборУслуг = "Все услуги" Тогда
			СтрокаУсловия = "ИСТИНА";
		ИначеЕсли ВыборУслуг = "Выбранные услуги" Тогда
			СтрокаУсловия = "Счетчик.ВидУслуги В (&ВыбранныеУслуги)";
		Иначе
			СтрокаУсловия = "НЕ Счетчик.ВидУслуги В (&ВыбранныеУслуги)";
		КонецЕсли;
		Запрос.Текст = СтрЗаменить(Запрос.Текст, "&УсловиеОтбораПоУслуге", СтрокаУсловия);
