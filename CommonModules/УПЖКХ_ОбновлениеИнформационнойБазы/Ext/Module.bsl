...
Процедура ПриДобавленииОбработчиковОбновления(Обработчики) Экспорт
	...
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.72.4";
	Обработчик.Процедура           = "УПЖКХ_ОбновлениеИнформационнойБазы.ИнвертироватьКоэффициентыПерерасчетаВДокументахКорректировкаНачислений";
	Обработчик.НачальноеЗаполнение = Ложь;
	Обработчик.Комментарий         = НСтр("ru = 'Инвертирование значений реквизитов ""Значение понижения тарифа"" и ""Процент понижения начисления"" в документе ""Корректировка начислений""'");
	
	Обработчик = Обработчики.Добавить();
	Обработчик.Версия              = "3.0.72.7";
	Обработчик.Процедура = "УПЖКХ_ОбновлениеИнформационнойБазы.ВыполнитьПропущенныеПриПереходеС20Процедуры";
	
	// Новые обработчики нужно заполнять копированием данного шаблона! Шаблон не изменять!!!
	//Обработчик = Обработчики.Добавить();
	//Обработчик.Версия              = "";
	//Обработчик.Процедура           = "УПЖКХ_ОбновлениеИнформационнойБазы.";
	// Если обработчик должен выполняться при переходе с предыдущей редакции,
	// то добавляем его вызов в процедуру ВыполнитьПереходСРедакции20.
	// Если при первоначальном заполнении ИБ - в ДействияПриНачальномЗаполненииИнформационнойБазы.
	//Обработчик.НачальноеЗаполнение = Ложь;                              // для отложенных обработчиков
	//Обработчик.Комментарий         = НСтр("ru = ''");                   // для отложенных обработчиков
	//Обработчик.БлокируемыеОбъекты  = "";                                // для отложенных обработчиков
	//Обработчик.ПроцедураПроверки   = "";                                // для отложенных обработчиков
	//Обработчик.РежимВыполнения     = "Отложенно";                       // для отложенных обработчиков
	//Обработчик.Идентификатор       = Новый УникальныйИдентификатор(""); // для отложенных обработчиков, сгенерировать
...
// УСТАРЕЛА, использовать функцию "ЗаписатьДанные".
...
//
Функция ЗаписатьДанные(Знач Данные, Знач РегистрироватьНаУзлахПлановОбмена = Неопределено, Знач ВключитьБизнесЛогику = Ложь)
...
Процедура ВыполнитьПереходСРедакции20(Параметры = Неопределено) Экспорт
	...
	ЗаполнитьЗначениеРеквизитаАдресЭлектроннойПочтыЛицевогоСчета_3_0_64_11();
	...
	ДействияПриОбновленииНа_3_0_65_9();
	...
	ЗаполнитьКодОрганизацииДляQRКодаВНастройкахФормированияПлатежногоДокумента(); // 3.0.70.3
	ДобавитьПараметрКодОрганизацииВСоставШКВНастройкахФормированияПлатежногоДокумента(); // 3.0.70.3
	ДействияВПодсистемеСМСПриОбновленииНа_3_0_70_6();
	УстановитьРасписаниеСлужебногоРеглЗаданияПолученияВажныхСообщенийИВключитьНапоминанияПользователей(); // 3.0.70.7
	ЗаполнитьКлючевыеОперацииЖКХ(); // 3.0.70.11
	ЗаполнитьЗначениеРеквизитаВариантКорректировкиУслуг(); // 3.0.71.4
	ИзменитьФормуПлатежногоДокументаВНастройкахФормированияПлатежногоДокумента(); // 3.0.71.8
	ИнвертироватьКоэффициентыПерерасчетаВДокументахКорректировкаНачислений(); // 3.0.72.4
...
Функция ПроверитьВозможностьПереходаСРедакции20(НомерВерсии20, НомерВерсии30)
	...
	Если НомерВерсии20 = "2.0.66.59" Тогда
		НомерВерсии30_Соответствующая = "3.0.72.4";
	...
	Если НомерВерсии20 = "2.0.66.60" Тогда
		НомерВерсии30_Соответствующая = "3.0.73.2";
...
Процедура ЗаполнитьПолеКоэффициентВРегистреСведенийУслугиОбщедомовогоСчетчика_3_0_64_8() Экспорт
	...
	// Имя параметра сохранения результатов выполнения данного обработчика.
	ИмяПараметраОбновленияИБ = "ЗаполнитьПолеКоэффициентВРегистреСведенийУслугиОбщедомовогоСчетчика_3_0_64_8";
	
	// Если обработка была выполнена ранее, то прекращаем выполнение данного обработчика обновления.
	ОбработкаОбновленияВыполненаРанее = ПолучитьПараметрОбновленияИБ(ИмяПараметраОбновленияИБ);
	Если ОбработкаОбновленияВыполненаРанее = Истина Тогда
		Возврат;
	КонецЕсли;
	...
	Если ОбработчикВыполненУспешно Тогда
		СохранитьПараметрОбновленияИБ(ИмяПараметраОбновленияИБ, Истина);
		ЗафиксироватьТранзакцию();
	Иначе
		ОтменитьТранзакцию();
...
Процедура ИспользоватьРассылкуКвитанцийПоЭлектроннойПочте_3_0_66_6() Экспорт
	...
	// Имя параметра сохранения результатов выполнения данного обработчика.
	ИмяПараметраОбновленияИБ = "ИспользоватьРассылкуКвитанцийПоЭлектроннойПочте_3_0_66_6";
...
Процедура ЗаполнитьРеквизитВариантНачисленияУслуги_3_0_67_9() Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если ВариантыНачисленияУслуги = Перечисления.УПЖКХ_ВариантыНачисленияУслуги.ПустаяСсылка() Тогда
			Продолжить;
		КонецЕсли;
...
Процедура ЗаполнитьНастройкиЗагрузкиВыгрузкиДляУниверсальнойВыгрузкиДанных_3_0_68_2() Экспорт
	...
	|	И КВП_НастройкиЗагрузкиВыгрузкиНастройкиСоответствияПолей.ТипПараметраБлокаСведений = ЗНАЧЕНИЕ(Перечисление.УПЖКХ_ТипыПараметровБлоковСведенийУниверсальнойВыгрузкиДанных.ПустаяСсылка)
...
Процедура УстановитьРасписаниеСлужебногоРеглЗаданияПолученияВажныхСообщенийИВключитьНапоминанияПользователей() Экспорт
	...
	// Имя параметра сохранения результатов выполнения данного обработчика.
	ИмяПараметраОбновленияИБ = "УстановитьРасписаниеСлужебногоРеглЗаданияПолученияВажныхСообщенийИВключитьНапоминанияПользователей";
...
Процедура ЗаполнитьЗначениеРеквизитаВариантКорректировкиУслуг(Параметры = Неопределено) Экспорт
	
	// В случае, если параметры не переданы в процедуру, либо не заполнены, обработчик выполняем
	...
	Если Параметры = Неопределено Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗаполнитьРеквизитВариантКорректировкиУслугДокумента(ДокументОбъект, Выборка);
	...
	Иначе
		...
		Параметры.ПрогрессВыполнения.ОбработаноОбъектов = Параметры.ПрогрессВыполнения.ОбработаноОбъектов + ОбработаноВТекущемПакете;
...
// Служебная процедура для заполнения реквизита ВариантКорректировкиУслуг в документе
// КВП_КорректировкаНачислений.
//
// Параметры:
//  ДокументСсылка - ДокументСсылка.КВП_КорректировкаНачислений - ссылка на документ.
//  Выборка        - ВыборкаИзРезультатаЗапроса - выборка, содержащая данные документа.
//
Процедура ЗаполнитьРеквизитВариантКорректировкиУслугДокумента(ДокументОбъект, Выборка)
...
КонецПроцедуры // ЗаполнитьРеквизитВариантКорректировкиУслугДокумента()
...
// Инвертирует значения реквизитов "Значение понижения тарифа" и "Процент понижения начисления" в документе "Корректировка начислений".
//
Процедура ИнвертироватьКоэффициентыПерерасчетаВДокументахКорректировкаНачислений() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	КВП_КорректировкаНачислений.Ссылка КАК Ссылка,
	|	КВП_КорректировкаНачислений.ВидОперации КАК ВидОперации
	|ИЗ
	|	Документ.КВП_КорректировкаНачислений КАК КВП_КорректировкаНачислений
	|ГДЕ
	|	(КВП_КорректировкаНачислений.ВидОперации = ЗНАЧЕНИЕ(Перечисление.КВП_ВидыОперацийКорректировкаНачислений.Недопоставка)
	|				И НЕ КВП_КорректировкаНачислений.УдалитьЗначениеПониженияТарифа = 0
	|				И КВП_КорректировкаНачислений.РазмерИзмененияТарифа = 0
	|			ИЛИ КВП_КорректировкаНачислений.ВидОперации = ЗНАЧЕНИЕ(Перечисление.КВП_ВидыОперацийКорректировкаНачислений.ПерерасчетПоПроценту)
	|				И НЕ КВП_КорректировкаНачислений.УдалитьПроцентПониженияНачисления = 0
	|				И КВП_КорректировкаНачислений.ПроцентИзмененияНачисления = 0)";
	
	Выборка = Запрос.Выполнить().Выбрать();
	Пока Выборка.Следующий() Цикл
		
		КорректировкаОбъект = Выборка.Ссылка.ПолучитьОбъект();
		
		// Инвертируем значения понижения, так как теперь положительное значение означает увеличение долга,
		// а отрицательное - уменьшение долга, что является логичным.
		Если Выборка.ВидОперации = Перечисления.КВП_ВидыОперацийКорректировкаНачислений.Недопоставка Тогда
			КорректировкаОбъект.РазмерИзмененияТарифа    = (-1) * КорректировкаОбъект.УдалитьЗначениеПониженияТарифа;
			
		ИначеЕсли Выборка.ВидОперации = Перечисления.КВП_ВидыОперацийКорректировкаНачислений.ПерерасчетПоПроценту Тогда
			КорректировкаОбъект.ПроцентИзмененияНачисления = (-1) * КорректировкаОбъект.УдалитьПроцентПониженияНачисления;
			
		КонецЕсли;
		
		Если Не ЗаписатьДанные(КорректировкаОбъект) Тогда
			ВызватьИсключениеПриОбновлении("Обработчик обновления ""ИнвертироватьКоэффициентыПерерасчетаВДокументахКорректировкаНачислений"" не выполнен!");
		КонецЕсли;
...
// Выполняет запуск необходимых при переходе с 2.0, но ранее не выполнявшихся
// обработчиков обновления. Выполнение процедур реализовано только для информационных баз,
// обновлявшихся с 2.0.
// 
Процедура ВыполнитьПропущенныеПриПереходеС20Процедуры() Экспорт
	
	// Запуск обработчиков обновления осуществляется только если информационная
	// база ранее обновлялась с версии 2.0.
	// Для проверки данного условия использоуется значение константы.
	
	НомерВерсииБППриПереходеС20 = Константы.УдалитьНомерВерсииКонфигурации.Получить();
	
	Если Не Лев(НомерВерсииБППриПереходеС20, 1) = "2" Тогда
		Возврат;
	КонецЕсли;
	
	// Для сокращения объема операций отсекаем процедуры обновления, которые нет необходимости
	// выполнять, т.к. они выполнялись ранее не в рамках перехода с 2.0,
	// а при обновлении внутри редакции 3.0.
	// Если переход с 2.0 выполнялся сравнительно давно, н-р, с версии ЖКХ 2.0.66.30
	// (соответствующая БП 2.0.66.47), тогда обновление выполнялось на версию ЖКХ не позднее 3.0.58.3,
	// значит, нет необходимости выполнять обработчики для более поздних версий ЖКХ 3.0.
	//
	// Список обработчиков к выполнению сформирован следующим образом:
	// Для каждого обработчика обновления определяем версию ЖКХ 3.0, с которой он начинает использоваться,
	// далее находим соответствующую ему версию ЖКХ 2.0. Список соответствий версий 2.0 и 3.0 хранится
	// в процедуре ПроверитьВозможностьПереходаСРедакции20 текущего модуля. Для версии обработчика берём
	// соответствующую, либо следующую версию 2.0. Далее для версии ЖКХ 2.0 определяем соответствующую
	// версию БП 2.0 - список хранится в процедуре НомерВерсииТиповойКонфигурации, найденная версия
	// БП 2.0 используется для сравнения с номером версии БП, хрянящемся в константе
	// УдалитьНомерВерсииКонфигурации.
	
	НомерВерсииБППриПереходеС20_Числом = ПолучитьНомерВерсииЧислом(НомерВерсииБППриПереходеС20);
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.47") Тогда
		ЗаполнитьДокументыНачисленияУслугДаннымиПоНачисленнымЗависимымУслугам_3_0_58_1();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.49") Тогда
		УстановитьЗначенияРесурсаНаименованиеПлатежаНазначениеВНастройкахФормированияПлатежногоДокумента_3_0_59_3();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.54") Тогда
		ЗаполнитьСостояниеПУДляСторноЗависимыхУслугВДокументахНачисленияУслуг_3_0_60_3();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.58") Тогда
		ДозаполнитьДокументыВводаПоказанийНовымиРеквизитамиТЧ_3_0_64_3();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.61") Тогда
		ЗаполнитьЗначениеРеквизитаАдресЭлектроннойПочтыЛицевогоСчета_3_0_64_11();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.64") Тогда
		ПерезаполнениеДокументовУправленияДоговорамиПоставщиковИДоговоровПоставщиков_3_0_65_6();
		ЗаполнитьЗначениеРесурсаЧислоНачалаПериодаПриемаПоказанийУчетнойПолитикиЖКХ_3_0_65_7();
		ЗаполнитьСоставДополнительныхРеквизитовQRПоКапитальномуРемонту_3_0_65_8();
		ДействияПриОбновленииНа_3_0_65_9();
	...
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.65") Тогда
		ВыполнитьПереносНастроекВСправочникУПЖКХ_НастройкиУниверсальнойВыгрузкиДанных_3_0_66_6();
		ЗаполнитьКодОрганизацииДляШтрихКодаПоУслугамКРВНастройкахФормированияПлатежногоДокумента_3_0_66_7();
		ДополнитьТабличнуюЧастьКорректировкиНачисленийУчитывающиеЗависимыеУслугиЗависимымиУслугами_3_0_66_7();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.71") Тогда
		ЗаполнитьРеквизитВариантНачисленияУслуги_3_0_67_9();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.73") Тогда
		ЗаполнитьНастройкиЗагрузкиВыгрузкиДляУниверсальнойВыгрузкиДанных_3_0_68_2();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.75") Тогда
		ЗаполнитьКодОрганизацииДляQRКодаВНастройкахФормированияПлатежногоДокумента(); // 3.0.70.3
		ДобавитьПараметрКодОрганизацииВСоставШКВНастройкахФормированияПлатежногоДокумента(); // 3.0.70.3
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.77") Тогда
		ДействияВПодсистемеСМСПриОбновленииНа_3_0_70_6();
	КонецЕсли;
	
	Если НомерВерсииБППриПереходеС20_Числом >= ПолучитьНомерВерсииЧислом("2.0.66.83") Тогда
		ЗаполнитьЗначениеРеквизитаВариантКорректировкиУслуг(); // 3.0.71.4
		ИзменитьФормуПлатежногоДокументаВНастройкахФормированияПлатежногоДокумента(); // 3.0.71.8
	КонецЕсли;
	
КонецПроцедуры // ВыполнитьПропущенныеПриПереходеС20Процедуры()
