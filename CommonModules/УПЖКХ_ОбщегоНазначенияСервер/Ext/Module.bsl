...
// Формирует границу интервала значений.
//
// Параметры:
//  ГраничноеЗначение - МоментВремени - дата и ссылка документа, для которого формируется граница.
//  ВидГраницы        - ВидГраницы - признак включения/исключения граничного значения в интервал.
//
// Возвращаемое значение:
//  Граница - Граница - граница интервала значений.
//
Функция ПолучитьГраницуДляПовышенияПроизводительности(ГраничноеЗначение, ВидГраницы) Экспорт
	
	// В некоторых случаях на большом объеме данных возможны ситуации, когда
	// использование созданной на основе ссылки на документ границы
	// в выборке данных приводит к существенному замедлению выполнения запроса.
	// В таких ситуациях можно создавать границу на основе только даты.
	//Если ТипЗнч(ГраничноеЗначение) = Тип("МоментВремени") Тогда
	//	Граница = Новый Граница(ГраничноеЗначение.Дата, ВидГраницы);
	//Иначе
	//	Граница = Новый Граница(ГраничноеЗначение, ВидГраницы);
	//КонецЕсли;
...
КонецФункции // ПолучитьГраницуДляПовышенияПроизводительности()
...
// Получает количество помещений в базе.
//
Функция ПолучитьКоличествоПомещенийВБазе() Экспорт
	
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	КОЛИЧЕСТВО(*) КАК КоличествоПомещений
	|ИЗ
	...
	Выборка = Запрос.Выполнить().Выбрать();
	Если Выборка.Следующий() Тогда
		Результат = Выборка.КоличествоПомещений;
	КонецЕсли;
