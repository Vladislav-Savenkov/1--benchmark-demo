...
Процедура УстановитьВидимость()
	...
	Элементы.ИмяФайлаВыгрузки.Видимость = РасширениеПодключено;
...
Процедура УстановитьДоступность()
	...
	Элементы.ВыгружатьСведенияПоЗакрытымЛицевымСчетам.Видимость = Объект.ВариантВыгрузки = 13;
...
Процедура ЗаполнитьДанныеВХранилище()
	...
	СтруктураПараметров.Вставить("ВыгружатьСведенияПоЗакрытымЛицевымСчетам", Объект.ВыгружатьСведенияПоЗакрытымЛицевымСчетам);
...
Процедура ИнициализацияРеквизитов()
	...
	Если НЕ Объект.НеЗаполнятьНастройкиПриОткрытии Тогда 
		...
		Если ТипЗнч(СтруктураПараметров) = Тип("Структура") Тогда
			...
			СтруктураПараметров.Свойство("ВыгружатьСведенияПоЗакрытымЛицевымСчетам", Объект.ВыгружатьСведенияПоЗакрытымЛицевымСчетам);
...
Функция ВыгрузитьСведенияВФайл(ИмяФайла)
	...
	Если Объект.ВариантВыгрузки = 11 Тогда
		ИмяФайла = ?(РасширениеПодключено,Объект.ИмяФайлаВыгрузки + "\","") + ?(ЗначениеЗаполнено(Объект.КодОСБ + Объект.НомерДоговора), Объект.КодОСБ + Объект.НомерДоговора, "temp") + ".zip";
	ИначеЕсли Объект.ВариантВыгрузки = 13 Тогда
		...
		Иначе
			ИмяФайла = ?(РасширениеПодключено,Объект.ИмяФайлаВыгрузки + "\","") + СокрЛП(Объект.Организация.ИНН) + "_" + СокрЛП(Объект.Организация.ОсновнойБанковскийСчет.НомерСчета) + "_" + ПорядковыйНомерБЗ + "_" + ПостоянныйСимволОрганизации + Формат(Объект.Дата, "ДФ=дд") + ".txt";
		КонецЕсли;
	ИначеЕсли Объект.ВариантВыгрузки = 7 Тогда
		ИмяФайла = ?(РасширениеПодключено,Объект.ИмяФайлаВыгрузки + "\", "") + "DB"+Формат(УПЖКХ_ТиповыеМетодыКлиентСервер.ПолучитьРабочуюДату(), "ДФ=ddMMyy") + ".VEK";
	ИначеЕсли Объект.ВариантВыгрузки = 9 ИЛИ Объект.ВариантВыгрузки = 10 Тогда
		ИмяФайла = ?(РасширениеПодключено,Объект.ИмяФайлаВыгрузки + "\","")+"OutFile.dbf";
	Иначе
		ИмяФайла = ?(РасширениеПодключено,Объект.ИмяФайлаВыгрузки + "\","")+"OutFile.txt";
	КонецЕсли;
	
	АдресВоВременномХранилище = СФормироватьФайл(ТаблицаСведений);
	
	СоответствиеИменФайловИАдресов.Вставить(ИмяФайла, АдресВоВременномХранилище);
...
Функция ПолучитьТаблицуСведений(МассивСписковЛС)
	...
	Для Каждого ТекСписокЛС ИЗ МассивСписковЛС Цикл
		...
		ИначеЕсли Объект.ВариантВыгрузки = 13 Тогда
			...
			Запрос.УстановитьПараметр("ВыгружатьСведенияПоЗакрытымЛицевымСчетам", Объект.ВыгружатьСведенияПоЗакрытымЛицевымСчетам);
			...
			|	КВП_ЛицевыеСчета.ЛицевойСчет.Код,
			|	КВП_ЛицевыеСчета.ЛицевойСчет.Наименование,
			|	КВП_ЛицевыеСчета.ЛицевойСчет.Идентификатор,
			...
			|ГДЕ
			|	КВП_ЛицевыеСчета.Период >= &ДатаНач
			|	И КВП_ЛицевыеСчета.Период <= &ДатаКон
			|	И КВП_ЛицевыеСчета.ЛицевойСчет В(&СписокЛС)
			|
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
...
Функция ВыгрузитьДанныеПоУслугамКапРемонтаВФайлыВариант13(ТаблицаСведений)
	...
	Для Каждого ТекРС ИЗ ТаблицаРСПоЗданиям Цикл
		...
		Если НЕ ПустаяСтрока(ТекРС.РасчетныйСчет) Тогда
			...
			ИмяФайла = ?(РасширениеПодключено,Объект.ИмяФайлаВыгрузки + "\","") + СокрЛП(Объект.Организация.ИНН) + "_" + СокрЛП(ТекРС.РасчетныйСчет) + "_" + ПорядковыйНомерБЗ + "_" + ПостоянныйСимволОрганизации + Формат(Объект.Дата, "ДФ=дд") + ".txt";
...
Функция СФормироватьФайл(ТаблицаСведений)
	...
	Если Объект.ВариантВыгрузки = 9 ИЛИ Объект.ВариантВыгрузки = 10 Тогда
		ФайлДБФ.ЗакрытьФайл();
		ИмяФайлаДляЗаписи = ИмяВременногоФайла;
	Иначе
		...
		Иначе
			ИмяВременногоФайла = КаталогВременныхФайлов() + СокрЛП(Объект.Организация.ИНН) + "_" + СокрЛП(Объект.РассчетныйСчет) + "_" + ?(Объект.Кодировка = 1, "WIN", "DOS") + ".txt";
			Текст.Записать(ИмяВременногоФайла, ?(Объект.Кодировка = 1, КодировкаТекста.ANSI, КодировкаТекста.OEM));
			...
			ИмяФайлаДляЗаписи = ИмяФайлаАрхива;
			
		КонецЕсли;
		
	КонецЕсли;
	
	ДвоичныеДанные = Новый ДвоичныеДанные(ИмяФайлаДляЗаписи);
...
&НаКлиенте
Процедура ВыгрузитьСведенияЗавершение(Результат = Неопределено, ДополнительныеПараметры = Неопределено) Экспорт
	...
	МассивВыгружаемыхФайлов = ВыгрузитьСведенияВФайл(ИмяФайла);
	ОбработчикЗавершения = Новый ОписаниеОповещения("ВыгрузитьСведенияЗавершениеФрагмент",ЭтотОбъект);
...
&НаКлиенте
Процедура ВыгрузитьСведенияЗавершениеФрагмент(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
