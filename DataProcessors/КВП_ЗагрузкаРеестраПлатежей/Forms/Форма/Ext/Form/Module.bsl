...
//
Процедура ЗагрузитьРеестр(Результат, ДополнительныеПараметры) Экспорт
	
	Для каждого СтрокаМассива Из Результат Цикл
			ЗагрузитьРеестрНаСервере(СтрокаМассива.Хранение, СтрокаМассива.Имя);
	КонецЦикла;
...
//
Процедура ЗагрузитьФайлXLS(Результат, ДополнительныеПараметры) Экспорт
	
	Для каждого СтрокаМассива Из Результат Цикл
			ЗагрузитьФайлXLSНаСервере(СтрокаМассива.Хранение, СтрокаМассива.Имя);
	КонецЦикла;
...
//
Процедура ЗагрузитьРеестрНаСервере(АдресВременногоХранилища, ИмяФайла) Экспорт
	...
	Сч = 1;
	врДанныеФайла = ПолучитьИзВременногоХранилища(АдресВременногоХранилища);
...
КонецПроцедуры // ЗагрузитьРеестрНаСервере()
...
Процедура ЗагрузитьФайлXLSНаСервере(АдресВременногоХранилища, ИмяФайла)
	...
	Попытка
		...
		СтрукрураИмениФайла = ОбщегоНазначенияКлиентСервер.РазложитьПолноеИмяФайла(ИмяФайла);
		
		ПутьКВременномуФайлу = КаталогВременныхФайлов() + СтрукрураИмениФайла.Имя;
	...
	ИначеЕсли ТипЗнч(ЗначениеПоля) = Тип("Строка") Тогда
		...
		Если РазделительДаты = СтруктураЗначенийПолей.СпРазделителиПериода[5].Значение Тогда // "Без разделителя"
			...
			Для Каждого ФорматДаты ИЗ спФорматДаты Цикл
				...
				Если ПараметрДаты = "День" И (НЕ ФорматПериодаБезДней) Тогда
...
Процедура Настройка(Команда)
	...
	ОткрытьФорму("Обработка.КВП_ЗагрузкаРеестраПлатежей.Форма.ФормаНастройкиЗаполнения",Новый Структура("РасширениеПодключено", РасширениеПодключено),
	ЭтаФорма,,,,
...
Процедура КомандаЗагрузитьРеестрЗавершение(Существует, ДополнительныеПараметры) Экспорт
	...
	Если Объект.ЗагружатьИзКаталога Тогда
		ФайлНаДиске.НачатьПроверкуЭтоКаталог(Новый ОписаниеОповещения("ПроверкаНаСуществованиеКаталогаЗавершение", ЭтотОбъект, Новый Структура("ФайлНаДиске", ФайлНаДиске)));
		Возврат;
	Иначе
...
&НаКлиенте
Процедура ПроверкаНаСуществованиеКаталогаЗавершение(ЭтоКаталог, ДополнительныеПараметры) Экспорт
...
Процедура ЗагрузитьРеестрНажатиеПродолжение()
	...
	Если РасширениеПодключено И ЗначениеЗаполнено(ИмяФайла) Тогда
		...
		Если Объект.ЗагружатьИзКаталога Тогда
			...
			ОписаниеОповещения = Новый ОписаниеОповещения("НачатьПоискФайловЗавершение", ЭтотОбъект, ЭтоФайлExcel);
			
			Маска = "*." + ФорматФайлаЗагрузки;
			
			НачатьПоискФайлов(ОписаниеОповещения, ИмяФайла, Маска);
			Возврат;
...
Процедура НачатьПоискФайловЗавершение(НайденныеФайлы, ДополнительныеПараметры) Экспорт
	...
	ЭтоФайлExcel = ДополнительныеПараметры;
...
Процедура ЗагрузитьРеестрНажатиеПродолжениеЗагрузка(МассивАдресов, ЭтоФайлExcel)
	...
	Для Каждого СтрокаМассива Из МассивАдресов Цикл
		СоответствиеИменФайловИАдресов.Вставить(СтрокаМассива, ПоместитьВоВременноеХранилище(СтрокаМассива, Новый УникальныйИдентификатор));
	КонецЦикла;
	...
	Если ЭтоФайлExcel Тогда
		ОбработчикЗавершения = Новый ОписаниеОповещения("ЗагрузитьФайлXLS", ЭтаФорма, ЭтоФайлExcel);
	Иначе
		ОбработчикЗавершения = Новый ОписаниеОповещения("ЗагрузитьРеестр", ЭтаФорма, ЭтоФайлExcel);
	КонецЕсли;
