...
Процедура ЗагрузитьРеестр()
	...
	Если РасширениеПодключено И ЗначениеЗаполнено(ИмяФайла) Тогда
		...
		//МассивЗагружаемыхФайлов = Новый Массив;
		//МассивЗагружаемыхФайлов.Добавить(Новый ОписаниеПередаваемогоФайла(ИмяФайла, ПоместитьВоВременноеХранилище(Неопределено, Новый УникальныйИдентификатор)));
...
Процедура ОбработатьРезультатПомещенияФайла(ПомещенныеФайлы, ДополнительныеПараметры) Экспорт
	
	Если НЕ ПомещенныеФайлы = Неопределено И Не ПомещенныеФайлы.Количество() = 0 Тогда
		
		УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Чтение данных из файла завершено.");
		
		ЗагрузитьРеестрВДерево(ПомещенныеФайлы[0].Хранение);
		...
		Для Каждого ТекСтрока из ИсходныеСтрокиСчетчика Цикл
			...
			Иначе
				...
				ИначеЕсли ТекСтрока.ТарифностьСчетчика = Перечисления.КВП_ТарифностьСчетчиков.Трехтарифный
					...
					И ТекСтрока.ЭтоПустоеПиковоеПоказание Тогда
					УстановитьИспользование = Ложь;
				КонецЕсли;
			КонецЕсли;
...
Процедура ЗагрузитьДанныеИзXLSФайла(ВременныйФайл)
	...
	Если КоличествоКолонок > 999 Тогда
...
Процедура СформироватьДокументыВводаПоказаний(МассивСсылокДокументовДляСозданияКорректировки)
	...
	Если Не НайденныеСтроки.Количество() = 0 Тогда
		Для Каждого ТекСтрока Из НайденныеСтроки Цикл
			...
			"В строке №" + ТекСтрока.Номер + " не заполнена дата показаний счетчика. Документы не сформированы.");
		КонецЦикла;
		Возврат;
	КонецЕсли;
...
Процедура ИмяФайлаЗагрузкиНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	// Для работы с файлами в веб-клиенте необходимо подключить расширение для работы с файлами.
	#Если ВебКлиент Тогда
		
		ОписаниеОповещения = Новый ОписаниеОповещения("ПоказатьДиалогВыбораФайлаЗагрузкиВебКлиент", ЭтотОбъект, Неопределено);
		ОбщегоНазначенияКлиент.ПоказатьВопросОбУстановкеРасширенияРаботыСФайлами(ОписаниеОповещения);
		
	#Иначе
		
		ПоказатьДиалогВыбораФайлаЗагрузки();
...
&НаКлиенте
// Обрабатывает результат подключения расширения работы с файлами и показывает диалог выбора файла.
//
Процедура ПоказатьДиалогВыбораФайлаЗагрузкиВебКлиент(ПодключеноРасширениеРаботыСФайлами, ДополнительныеПараметры) Экспорт
	...
	Если РасширениеРаботыСФайламиПодключено Тогда
		ПоказатьДиалогВыбораФайлаЗагрузки();
	Иначе
		УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьОбОшибке("Не удалось подключить расширение работы с файлами.");
	КонецЕсли;
	
КонецПроцедуры

&НаКлиенте
// Показывает диалог выбора файла загрузки.
//
Процедура ПоказатьДиалогВыбораФайлаЗагрузки()
...
&НаКлиенте
// Обработчик оповещения события "НачалоВыбора" поля "ИмяФайлаЗагрузки".
//
