...
Процедура ДобавитьАвтоВложения()
	...
	//Квартплата+
	СтрокиИнформацииДляТехническойПоддержки   = Объект.Вложения.НайтиСтроки(Новый Структура("Идентификатор", "ИнформацияДляТехническойПоддержкиЖКХ"));
	ДобавитьИнформацюДляТехническойПоддержки = (СтрокиИнформацииДляТехническойПоддержки.Количество() = 0);
	
	Если ДобавитьИнформацюДляТехническойПоддержки Тогда
		// Добавляем свои настройки программы.
		ИмяФайлаИнформацииДляТехническойПоддержки = ПолучитьИмяВременногоФайла("txt");
		
		ТекстовыйДокумент = ОТР_ОбращениеВТехническуюПоддержкуПереопределяемый.ИнформацияДляТехническойПоддержкиЖКХ(Организация, ВидПриложения);
		ТекстовыйДокумент.Записать(ИмяФайлаИнформацииДляТехническойПоддержки);
		
		НовоеВложение = Объект.Вложения.Добавить();
		НовоеВложение.Представление             = НСтр("ru='Информация для технической поддержки.txt'");
		НовоеВложение.Идентификатор             = "ИнформацияДляТехническойПоддержкиЖКХ";
		НовоеВложение.АдресВоВременномХранилище = ПоместитьВоВременноеХранилище(Новый ДвоичныеДанные(ИмяФайлаИнформацииДляТехническойПоддержки), УникальныйИдентификатор);
		
	КонецЕсли;
	
	// Стандартные вложения не добавляем.
	Возврат;
