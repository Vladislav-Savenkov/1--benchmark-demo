...
Процедура ПолучитьТаблицуНачисленийВолгоград(ТаблицаДанныхФайлаОНачислениях, ТаблицаАдресов, ТаблицаФИО)
	...
	|	ТаблицаИзФайла.NORM_GH,
	|	ТаблицаИзФайла.TKO,
	|	ТаблицаИзФайла.NORM_TKO,
	|	ТаблицаИзФайла.TARIF_TKO,
	|	ТаблицаИзФайла.MES_TKO
	|ПОМЕСТИТЬ втТаблицаДанныхИзФайла
	|ИЗ
	...
	|		КОНЕЦ) КАК NORM_GH,
	|	СУММА(ВЫБОР
	|			КОГДА втИтоговыеНачисления.ИдентификаторУслуги = ""TKO""
	|				ТОГДА втИтоговыеНачисления.СуммаНачисления
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК TKO,
	|	МАКСИМУМ(ВЫБОР
	...
	|	ЕСТЬNULL(втИтоговыеНачисленияПоУслугам.NORM_GH, ЗНАЧЕНИЕ(Справочник.КВП_НормыПотребленияУслуг.ПустаяСсылка)) КАК NORM_GH,
	|	ЕСТЬNULL(втИтоговыеНачисленияПоУслугам.TKO, 0) КАК TKO,
	|	ЕСТЬNULL(втИтоговыеНачисленияПоУслугам.NORM_TKO, ЗНАЧЕНИЕ(Справочник.КВП_НормыПотребленияУслуг.ПустаяСсылка)) КАК NORM_TKO,
	|	ЕСТЬNULL(втИтоговыеНачисленияПоУслугам.TARIF_TKO, 0) КАК TARIF_TKO
	...
	|	ЕСТЬNULL(втИтоговыеНачисленияПоУслугам.NORM_GH, ЗНАЧЕНИЕ(Справочник.КВП_НормыПотребленияУслуг.ПустаяСсылка)),
	|	ЕСТЬNULL(втИтоговыеНачисленияПоУслугам.TKO, 0),
	|	ЕСТЬNULL(втИтоговыеНачисленияПоУслугам.NORM_TKO, ЗНАЧЕНИЕ(Справочник.КВП_НормыПотребленияУслуг.ПустаяСсылка)),
	|	ЕСТЬNULL(втИтоговыеНачисленияПоУслугам.TARIF_TKO, 0)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	втТаблицаДанныхИзФайла.NORM_GH,
	|	ЕСТЬNULL(втПолученныеДанныеДляВыгрузки.NORM_GH, ЗНАЧЕНИЕ(Справочник.КВП_НормыПотребленияУслуг.ПустаяСсылка)) КАК NORM_GHИзБазы,
	|	ЕСТЬNULL(втПолученныеДанныеДляВыгрузки.TKO, 0) КАК TKO,
	|	втТаблицаДанныхИзФайла.NORM_TKO,
	|	ЕСТЬNULL(втПолученныеДанныеДляВыгрузки.NORM_TKO, ЗНАЧЕНИЕ(Справочник.КВП_НормыПотребленияУслуг.ПустаяСсылка)) КАК NORM_TKOИзБазы,
	|	ЕСТЬNULL(втПолученныеДанныеДляВыгрузки.TARIF_TKO, 0) КАК TARIF_TKO,
	|	втТаблицаДанныхИзФайла.MES_TKO,
	...
	Для Каждого СтрокаРезультата ИЗ ТаблицаДанныхФайлаОНачислениях Цикл
		...
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаРезультата.NORM_GHИзБазы) Тогда
			Если НЕ СтрокаРезультата.NORM_GHИзБазы.ВариантНастройки = ПредопределенноеЗначение("Перечисление.КВП_ВариантыНастроекРасчетаНормативовПотребления.Простой") Тогда
				СтрокаРезультата.NORM_GH = ПолучитьРазмерНормыПотребления(СтрокаРезультата.ЛицевойСчет, КонецМесяца(Период), СтрокаРезультата.NORM_GHИзБазы, Организация);
			Иначе
				СтрокаРезультата.NORM_GH = СтрокаРезультата.NORM_GИзБазы.Размер;
			КонецЕсли;
		Иначе
			СтрокаРезультата.NORM_GH = 0;
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаРезультата.NORM_TKOИзБазы) Тогда
			Если НЕ СтрокаРезультата.NORM_TKOИзБазы.ВариантНастройки = ПредопределенноеЗначение("Перечисление.КВП_ВариантыНастроекРасчетаНормативовПотребления.Простой") Тогда
				СтрокаРезультата.NORM_TKO = ПолучитьРазмерНормыПотребления(СтрокаРезультата.ЛицевойСчет, КонецМесяца(Период), СтрокаРезультата.NORM_TKOИзБазы, Организация);
			Иначе
				СтрокаРезультата.NORM_TKO = СтрокаРезультата.NORM_GИзБазы.Размер;
			КонецЕсли;
		Иначе
			СтрокаРезультата.NORM_TKO = 0;
		КонецЕсли;
	КонецЦикла;
