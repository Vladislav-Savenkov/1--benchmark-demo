...
Процедура ПолучитьСпискиУслуг()
	...
	СписокУслуг.Добавить("TKO",      "Обращение с ТКО");
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ВолгоградскаяОбласть_ТаблицаУслуг.Количество() = 0 Тогда
		ПолучитьСпискиУслуг();
	ИначеЕсли Объект.ВолгоградскаяОбласть_ТаблицаУслуг.НайтиСтроки(Новый Структура("Идентификатор", "TKO")).Количество() = 0 Тогда
		НоваяУслуга = Объект.ВолгоградскаяОбласть_ТаблицаУслуг.Добавить();
		НоваяУслуга.Услуга        = "Обращение с ТКО";
		НоваяУслуга.Идентификатор = "TKO";
	КонецЕсли;
