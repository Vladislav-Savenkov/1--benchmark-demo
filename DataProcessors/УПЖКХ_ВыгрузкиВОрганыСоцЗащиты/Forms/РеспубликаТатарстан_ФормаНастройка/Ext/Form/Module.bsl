...
&НаСервере
// Процедура производит изменения в "РеспубликаТатарстан_ТаблицаНастроек" и в "РеспубликаТатарстан_УслугиВБазе" в соответствии с изменениями макета.
//
Процедура СравнитьУслугиВОСЗН()
		
		Макет = РеквизитФормыВЗначение("Объект").ПолучитьМакет("РеспубликаТатарстан_Макет");
		ОбластьСправочникУслуг = Макет.ПолучитьОбласть("СправочникУслуг");
		УслугиВМакетеМассив = Новый Массив;
		ТаблицаДляСортировки = Новый ТаблицаЗначений;
		ОписаниеТипа = Новый ОписаниеТипов("Число");
		
		Для НомерСтроки = 1 По ОбластьСправочникУслуг.ВысотаТаблицы Цикл
			КодВОСЗНМакет = ОбластьСправочникУслуг.Область(НомерСтроки, 1).Текст;
			УслугаВОСЗНМакет = ОбластьСправочникУслуг.Область(НомерСтроки, 2).Текст;
			ОтборКод = Новый Структура;
			ОтборКод.Вставить("КодУслугиВОСЗН", КодВОСЗНМакет);
			
			// Ищем совпадение в таблице РеспубликаТатарстан_ТаблицаНастроек и перезаписываем наименование услуги.
			СтрокиСовпаденияКод = Объект.РеспубликаТатарстан_ТаблицаНастроек.НайтиСтроки(ОтборКод);
			Для Каждого СравнениеПоКоду ИЗ СтрокиСовпаденияКод Цикл
				
				СравнениеПоКоду.УслугаВОСЗН = УслугаВОСЗНМакет;
				
			КонецЦикла;
			
			// Если совпадений нет, тогда создаем новую запись с кодом и наименованием услуги.
			Если СтрокиСовпаденияКод.Количество() = 0 Тогда
				
				ДобавитьКодУслугиВОСЗН = Объект.РеспубликаТатарстан_ТаблицаНастроек.Добавить();
				ДобавитьКодУслугиВОСЗН.КодУслугиВОСЗН = КодВОСЗНМакет;
				ДобавитьКодУслугиВОСЗН.УслугаВОСЗН = УслугаВОСЗНМакет;
				
			КонецЕсли;
			
			// Ищем совпадение в таблице РеспубликаТатарстан_УслугиВБазе и перезаписываем наименование услуги.
			ПоискСоответствийПоКодуРеспубликаТатарстан_УслугиВБазе = Объект.РеспубликаТатарстан_УслугиВБазе.НайтиСтроки(ОтборКод);
			Для Каждого УслугаВОСЗНРеспубликаТатарстан_УслугиВБазе ИЗ ПоискСоответствийПоКодуРеспубликаТатарстан_УслугиВБазе Цикл
				
				УслугаВОСЗНРеспубликаТатарстан_УслугиВБазе.УслугаВОСЗН = УслугаВОСЗНМакет;
				
			КонецЦикла;
			
			УслугиВМакетеМассив.Добавить(КодВОСЗНМакет);
			
		КонецЦикла;
		
		// Ищем совпадение в массиве, если совпадений нет - удаляем строку.
		Для Каждого КодУслугиВТаблицеНастроек из Объект.РеспубликаТатарстан_ТаблицаНастроек Цикл
			ПоискУслугВМассиве = УслугиВМакетеМассив.Найти(КодУслугиВТаблицеНастроек.КодУслугиВОСЗН);
			
			Если ПоискУслугВМассиве = Неопределено Тогда
				
				Объект.РеспубликаТатарстан_ТаблицаНастроек.Удалить(КодУслугиВТаблицеНастроек);
		...
		// Ищем совпадение в массиве, если совпадений нет - удаляем строку.
		Для Каждого КодУслугиВТаблицеУслугаВБазе из Объект.РеспубликаТатарстан_УслугиВБазе Цикл
			ПоискУслугВМассиве = УслугиВМакетеМассив.Найти(КодУслугиВТаблицеУслугаВБазе.КодУслугиВОСЗН);
			
			Если ПоискУслугВМассиве = Неопределено Тогда
				
				Объект.РеспубликаТатарстан_УслугиВБазе.Удалить(КодУслугиВТаблицеУслугаВБазе);
				
			КонецЕсли;
		
		КонецЦикла;
		
		// Сортируем РеспубликаТатарстан_ТаблицаНастроек по коду услуги.
		ТаблицаДляСортировки = Объект.РеспубликаТатарстан_ТаблицаНастроек.Выгрузить();
		ТаблицаДляСортировки.Колонки.Добавить("КодУслугиВОСЗН_Число", ОписаниеТипа);
		ТаблицаДляСортировки.ЗагрузитьКолонку(ТаблицаДляСортировки.ВыгрузитьКолонку("КодУслугиВОСЗН"),"КодУслугиВОСЗН_Число");
		ТаблицаДляСортировки.Сортировать("КодУслугиВОСЗН_Число Возр");
		Объект.РеспубликаТатарстан_ТаблицаНастроек.Загрузить(ТаблицаДляСортировки);
		
КонецПроцедуры // СравнитьУслугиВОСЗН()
...
Процедура КомандаОбновитьСписокУслуг(Команда)
	...
	Если Объект.РеспубликаТатарстан_УслугиВБазе.Количество() > 0 Тогда
		...
		ТекстВопроса = "Очистить установленные ранее соответствия услуг?";
...
Процедура ТП_ЖКУ_ВБазеПередУдалением(Элемент, Отказ)
	...
	УслугаВБазе   = Элементы.ТП_ЖКУ_ВБазе.ТекущиеДанные.УслугаВБазе;
	ТипНачисления = Элементы.ТП_ЖКУ_ВБазе.ТекущиеДанные.ТипНачисления;
	
	Отбор = Новый Структура;
	Отбор.Вставить("Идентификатор",  Идентификатор);
	Отбор.Вставить("УслугаВБазе",    УслугаВБазе);
	Отбор.Вставить("ТипНачисления", ТипНачисления);
...
Процедура ТП_ЖКУ_ВБазеПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если Не НоваяСтрока Тогда
		ПредыдущаяУслуга = Элементы.ТП_ЖКУ_ВБазе.ТекущиеДанные.УслугаВБазе;
		ПредыдущийТипНачисления = Элементы.ТП_ЖКУ_ВБазе.ТекущиеДанные.ТипНачисления;
	Иначе
	КонецЕсли;
...
Процедура ТП_ЖКУ_ВБазеПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	...
	Если Не Элемент.ТекущиеДанные = Неопределено И Не ОтменаРедактирования Тогда
		...
		Иначе
			...
			НоваяСтрокаСоответствия.ТипНачисления  = ТипНачисления;
			
			// Удаляем старую запись при перевыборе.
			Если Не ПредыдущаяУслуга = УслугаВБазе 
				ИЛИ НЕ ПредыдущийТипНачисления = ТипНачисления Тогда
				
				Отбор = Новый Структура;
				Отбор.Вставить("Идентификатор", Идентификатор);
				Отбор.Вставить("УслугаВБазе",   ПредыдущаяУслуга);
				Отбор.Вставить("ТипНачисления", ПредыдущийТипНачисления);
				
				НайденнаяСтрока = Объект.РеспубликаТатарстан_УслугиВБазе.НайтиСтроки(Отбор);
				Если НайденнаяСтрока.Количество() = 1 Тогда
					НайденнаяСтрока[0].УслугаВБазе = УслугаВБазе;
					НайденнаяСтрока[0].ТипНачисления = ТипНачисления;
				КонецЕсли;
...
&НаКлиенте
Процедура ТП_ЖКУ_ВБазеТипНачисленияПриИзменении(Элемент)
	ЭтаФорма.ТекущийЭлемент = Элементы.ТаблицаНастроек;
КонецПроцедуры
