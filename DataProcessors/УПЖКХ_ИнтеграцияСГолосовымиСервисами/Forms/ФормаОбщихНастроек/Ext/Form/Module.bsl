...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	СохраненныеНастройки.Свойство("ВыборУслуг", ВыборУслуг);
	
	Если ПустаяСтрока(ВыборУслуг) Тогда
		ВыборУслуг = "Все услуги";
	КонецЕсли;
	
	Для Каждого ТекЭлСписка Из СохраненныеНастройки.ВыбранныеУслуги Цикл
		Если НЕ ТекЭлСписка.Значение.ПолучитьОбъект() = Неопределено Тогда
			ВыбранныеУслуги.Добавить().Услуга = ТекЭлСписка.Значение;
		КонецЕсли;
	КонецЦикла;
...
Процедура СохранитьНастройки()
	...
	СтруктураНастроек.ВыборУслуг                  = ВыборУслуг;
	
	СтруктураНастроек.ВыбранныеУслуги.Очистить();
	Для Каждого ТекСтрока Из ВыбранныеУслуги Цикл
		Если ЗначениеЗаполнено(ТекСтрока.Услуга) Тогда
			СтруктураНастроек.ВыбранныеУслуги.Добавить(ТекСтрока.Услуга);
		КонецЕсли;
	КонецЦикла;
...
&НаСервере
// Процедура управляет свйоствами элементов формы.
Процедура УправлениеФормой()
	
	Элементы.ВыбранныеУслуги.Видимость = НЕ (ВыборУслуг = "Все услуги");
