...
Процедура ВыполнитьВыгрузкуПлатежныхДокументов(Эксель, КоличествоФайлов, КаталогВыгрузки, Форма, СтруктураИменТаблицФормы, СформированныеФайлы, ФормируемНаСервере)
	...
	Для Партия = 1 По КоличествоПартий Цикл
		...
		Для Каждого СтрокаЛС Из СтрокиЛС Цикл
			...
			ЛистЛС.cells(Счетчик, 26).value = СтрЗаменить(СтрокаЛС.КРИтогоКОплате, ",", ".");
			
			// Пени.
			ЛистЛС.cells(Счетчик, 32).value = ЗаменитьНаПустуюСтроку(СтрокаЛС.СуммаПени);
			...
			ЛистЛС.cells(Счетчик, 37).value = СтрокаЛС.ИдентификаторПД;
...
Процедура ОбработатьФайлыНаСервере()
	...
	Для Каждого ТекИмя Из СтруктураИменФайлов Цикл
		...
		ОбластьКолонкаРазделы3_6   = ТабДок.НайтиТекст("Способ определения объемов КУ");
		
		Если ОбластьКолонкаИдентификаторПД = Неопределено Или ОбластьКолонкаРазделы3_6 = Неопределено Тогда
		...
		ПоследняяСтрока = ОбластьКолонкаРазделы3_6.Верх - 4;
...
Процедура ВыполнитьВыгрузкуПриборовУчета(Эксель, КоличествоФайлов, КаталогВыгрузки, МакетПУ, Форма, СтруктураИменТаблицФормы, СформированныеФайлы, ФормируемНаСервере)
	...
	Если ВидОбменаПУ = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыОбменаПриборамиУчетаСГИСЖКХ.ВыгрузкаСведенийПУ") Тогда
		...
		Для Каждого СтрокаСчетчика Из ОсновныеСтрокиСчетчиков Цикл
			...
			Если ЗначениеЗаполнено(СтрокаСчетчика.ДатаУстановки) Тогда
				ЛистСведенийОПУ.cells(Счетчик, 23).value = СтрокаСчетчика.ДатаУстановки;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаСчетчика.ДатаВводаВЭксплуатацию) Тогда
				ЛистСведенийОПУ.cells(Счетчик, 24).value = СтрокаСчетчика.ДатаВводаВЭксплуатацию;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаСчетчика.ДатаПоверки) Тогда
				ЛистСведенийОПУ.cells(Счетчик, 25).value = СтрокаСчетчика.ДатаПоверки;
			КонецЕсли;
			
			Если ЗначениеЗаполнено(СтрокаСчетчика.ДатаОпломбированияЗаводом) Тогда
				ЛистСведенийОПУ.cells(Счетчик, 26).value = СтрокаСчетчика.ДатаОпломбированияЗаводом;
			КонецЕсли;
	...
	Иначе
		...
		Для Каждого СтрокаСчетчика Из ТаблицаСчетчиков Цикл
			...
			Если ЗначениеЗаполнено(СтрокаСчетчика.ДатаПоказания) Тогда
				ЛистПоказаний.cells(Счетчик, 7).value = СтрокаСчетчика.ДатаПоказания;
			КонецЕсли;
