...
Процедура УправлениеФормой(Форма)
	...
	Иначе
		...
		Элементы.РазмерИзмененияТарифа.Видимость = Объект.ЧастичныйПерерасчет;
...
Процедура ЗаполнитьОснованиеПерерасчета()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение(
		...
		Если Объект.ФлагПониженияТарифа Тогда
			Объект.ОснованиеПерерасчета = "" + Объект.ОснованиеПерерасчета + " с изменением тарифа";
		Иначе
			Объект.ОснованиеПерерасчета = "" + Объект.ОснованиеПерерасчета + " по проценту";
		КонецЕсли;
...
Процедура ОбработатьВыборУслуг(ВыбранноеЗначение, ДопПараметры)Экспорт
	...
	Если НЕ ВыбранноеЗначение = Неопределено Тогда
		...
		Иначе
			...
			ОбработатьВыборУслугНаСервере(ВыбранноеЗначение, ДопПараметры);
...
&НаСервере
// Обработчик выбора услуги на сервере.
//
Процедура ОбработатьВыборУслугНаСервере(ВыбранноеЗначение, ДопПараметры) Экспорт
	...
	Если СтрокиСоставнойУслуги.Количество() > 0 Тогда
		...
		Для Каждого СтрокаСоставнойУслуги Из СтрокиСоставнойУслуги Цикл
			...
			ДопПараметры.Свойство("ЛицевойСчет"    , НоваяСтрока.ЛицевойСчет);
			ДопПараметры.Свойство("МесяцНачисления", НоваяСтрока.МесяцНачисления);
	...
	Иначе
		...
		ДопПараметры.Свойство("ЛицевойСчет"    , НоваяСтрока.ЛицевойСчет);
		ДопПараметры.Свойство("МесяцНачисления", НоваяСтрока.МесяцНачисления);
		ДопПараметры.Свойство("УслугаОснование", НоваяСтрока.УслугаОснование);
		ДопПараметры.Свойство("ТипНачисления"  , НоваяСтрока.ТипНачисления) 
...
КонецПроцедуры // ОбработатьВыборУслугСервер()
...
Процедура ВидПерерасчетаПриИзменении(Элемент)
	...
	ИначеЕсли Объект.ВидПерерасчета = 1 Тогда
		...
		Объект.РазмерИзмененияТарифа = 0;
...
&НаКлиенте
// Клиентская прослойка процедуры-обработчика команды "КомандаЗаполнитьПродолжение()" для работы на веб-клиенте.
//
Процедура ОкончаниеВыбораПолучателейУслуг(СписокВыбранныхОбъектов, ДополнительныеПараметры) Экспорт
	
	ОкончаниеВыбораПолучателейУслугНаСервере(СписокВыбранныхОбъектов, ДополнительныеПараметры);
...
// Процедура-продолжение процедуры-обработчика команды "КомандаЗаполнитьПродолжение()".
//
Процедура ОкончаниеВыбораПолучателейУслугНаСервере(СписокВыбранныхОбъектов, ДополнительныеПараметры)
