...
Процедура ЗаполнитьТабличнуюЧастьУслуги(СписокОбъектов) Экспорт
	...
	ИначеЕсли ВидОперации = Перечисления.КВП_ВидыОперацийНачисленияУслуг.НачисленияПоПоказаниямПУ Тогда
		...
		// При заполнении табличной части определяем, что списание расхода ПУ
		// будет выполняться по данным табличной части "Счетчики" документа.
		СписыватьРасходПУПоТаблицеСчетчиков = Истина;
		...
		Если НЕ СтрокаВетки.ВидОбъекта = "Помещение" Тогда // на нижестоящих уровнях нет коллективных приборов учета.
			...
			Иначе
				...
				ИначеЕсли СтрокаВетки.ФормулаРасчета = Документы.КВП_НачислениеУслуг.ФормулаРасчета_ПолучитьЗначениеПоФормулам2_3И2_5Постановления1708() Тогда
					...
					// Постановление 1708: дом не оборудован КПУ.
				...
				ИначеЕсли СтрокаВетки.ФормулаРасчета = Документы.КВП_НачислениеУслуг.ФормулаРасчета_ПолучитьЗначениеПоФормулам3И3_6Постановления1708() Тогда
					...
					// Постановление 1708: дом оборудован КПУ и нет ни одного помещения, оборудованного ИПУ.
					...
					Для Каждого СтрокаЛС ИЗ ЛицевыеСчета Цикл
						...
						Иначе
							...
							ЧислительИндивидуальныйОбъем = СтрокаЛС.Доля * КоличествоРаспределения;
							ЗнаменательИндивидуальныйОбъем = ЖилаяНежилаяПлощадь - ПлощадьСИндивидуальнымОтоплением + ПлощадьМОП;
					...
					// КоличествоРаспределения = [Показания ОПУ] - [Показания ИПУ по услугам базам-расчета].
					// Уменьшим показания ОПУ также на покзания ИПУ, рассчитанные только что по формуле 3(6).
					КоличествоРаспределения = КоличествоРаспределения - СуммарныйИндивидуальныйОбъем;
				...
				ИначеЕсли СтрокаВетки.ФормулаРасчета = Документы.КВП_НачислениеУслуг.ФормулаРасчета_ПолучитьЗначениеПоФормулам3_1И3_7Постановления1708() Тогда
					...
					// Постановление 1708: дом оборудован КПУ и хотя бы одно помещение оборудовано ИПУ.
					...
					// КоличествоРаспределения = [Показания ОПУ] - [Показания ИПУ] - [Показания ИПУ по услугам базам-расчета].
					// Уменьшим показания ОПУ также на покзания ИПУ, рассчитанные только что по формуле 3(7).
					КоличествоРаспределения = КоличествоРаспределения - СуммаОбъемовБезИПУ;
				...
				ИначеЕсли СтрокаВетки.ФормулаРасчета = Документы.КВП_НачислениеУслуг.ФормулаРасчета_ПолучитьЗначениеПоФормуле3_3Постановления1708() Тогда
					...
					// Постановление 1708: дом оборудован КПУ и все помещения оборудованы ИПУ.
...
Процедура РассчитатьПоказания() Экспорт
	...
	Для Каждого СтрокаРаспределения ИЗ ТаблицаРаспределения Цикл
		...
		Для Каждого ТекСтрока ИЗ СтрокиПоУслугам Цикл
			...
			Если НЕ ИндКоличествоРаспределения = 0 И Услуга = ТекСтрока.Услуга Тогда
...
Процедура ОпределитьСуммуОбъемовПоИПУИСуммуПлощадейПомещенийСИПУ(СтрокаВетки, СуммаОбъемовПоИПУ, ПлощадьПомещенийСИПУ)
	...
	// Для этого показания по ИПУ получаем из табличной части "УслугиПоПоказаниямПУ", а не из
	// переменной "ОбщийРасходПоИПУ", чтобы не учитывать объемы по услуге-базе расчета в случае,
	// когда начисления по основной услуге и услуге-базе расчета производится одним документом
	// (услуга общедомового счетчика).
	Запрос = Новый Запрос;
	Запрос.Текст =
	"ВЫБРАТЬ
	|	КВП_НачислениеУслугУслугиПоПоказаниямПУ.ЛицевойСчет КАК ЛицевойСчет,
	...
	|	&УслугиПоПоказаниямПУ КАК КВП_НачислениеУслугУслугиПоПоказаниямПУ
	|ГДЕ
	|	КВП_НачислениеУслугУслугиПоПоказаниямПУ.Услуга = &Услуга
	|	И НЕ КВП_НачислениеУслугУслугиПоПоказаниямПУ.Сторно
	|	И НЕ КВП_НачислениеУслугУслугиПоПоказаниямПУ.НачислениеПоПовышающемуКоэффициенту
	|	И НЕ КВП_НачислениеУслугУслугиПоПоказаниямПУ.Зависимая
	|;
	|
...
Процедура ДополнитьТаблицуУслугПоПоказаниямИндивидуальнымиПУ(ТЗУслугиПоПоказаниямПУ, ТаблицаЛС, ДопСписокУслуг)
	...
	|	ТаблицаЛС_с_ПУ.Услуга.ВариантыНачисленияУслуги КАК ВариантыНачисленияУслуги,
	...
	|	ТаблицаЛС_с_ПУ.Услуга.ВариантыНачисленияУслуги,
...
Процедура ЗаполнитьТаблицуПериодов(ТаблицаПериодов, СтрокаПУ, ЕстьИндивидуальныеПУВДоме, ЕстьИндивидуальныеПУВПодъезде,
	...
	Для Каждого СтрокаТекПериода Из ТаблицаПериодов Цикл
		...
		ИначеЕсли СтрокаПУ.ВариантыНачисленияУслуги = Перечисления.УПЖКХ_ВариантыНачисленияУслуги.Постановление1708
				  И НЕ СтрокаПУ.ЕстьДомовойСчетчик И НЕ СтрокаПУ.ЕстьПодъездныйСчетчик Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации = Перечисления.КВП_ВидыОперацийНачисленияУслуг.НачисленияПоПоказаниямПУ Тогда
		...
		// Доступны 2 способа списания расхода ПУ: по данным регистра накопления
		// "Расчет приборов учета" (устаревший подход),
		// по табличной части документа "Счетчики" (новый подход).
		// При создании новых документов устанавливаем признак использования нового подхода
		// в значение Истина для последующего корректного формирования движений.
		Если ЭтоНовый() Тогда
			СписыватьРасходПУПоТаблицеСчетчиков = Истина;
		КонецЕсли;
