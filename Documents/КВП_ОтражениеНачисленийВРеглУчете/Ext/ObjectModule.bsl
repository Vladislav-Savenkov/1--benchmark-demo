...
Процедура ЗаполнитьСчетаУчетаВСтрокеТабЧасти(СтрокаТЧ) Экспорт
	...
	Если СтрокаТЧ.ЭтоПени Тогда
		...
		Если (мНастройкиУчетнойПолитикиТСЖ.СчетаУчетаДляОтраженияПени = Перечисления.УПЖКХ_СчетаУчетаДляПениПриОтраженииВРеглУчете.ОтдельныеСчета
			 ...
			 И СтрокаТЧ.ЭтоКапРемонт) Тогда
			
			СчетаУчетаПени = Документы.КВП_ОтражениеНачисленийВРеглУчете.ПолучитьСтруктуруСчетаУчетаРасчетаПоПени(Организация, ВидВзаиморасчетов, ?(СтрокаТЧ.ЭтоКапРемонт, Истина, Ложь));
			...
			// По пени не может быть авансов, поэтому используем основной счет расчетов.
			СчетаУчета.СчетАвансовПокупателя  = СчетаУчета.СчетРасчетовПокупателя;
...
Процедура ЗаполнитьСчетаУчетаУслугТабличнойЧасти() Экспорт
	...
	Документы.КВП_ОтражениеНачисленийВРеглУчете.ЗаполнитьСчетаУчетаУслугТабличнойЧасти(Дата, Организация, РасшифровкаНачислений, мНастройкиУчетнойПолитикиТСЖ, мСчетаУчетаНоменклатуры);
...
Процедура ДобавитьСтрокуВТабличнуюЧастьДокументаРеализации(СтруктураПараметровСтроки)
	...
	УстановитьСчетаУчетаДляПени = СтруктураПараметровСтроки.УстановитьСчетаУчетаДляПени;
	
	Если УстановитьСчетаУчетаДляПени Тогда
		СчетаУчетаПени = СтруктураПараметровСтроки.СчетаУчетаПени;
	КонецЕсли;
	
	Если мЭтоРеализация И СтрокаРасшифровки.ВариантПоставкиУслуг = Перечисления.УПЖКХ_ВариантыРасчетовСПоставщикамиУслуг.АгентскаяСхема Тогда
	...
	Если мЭтоРеализация Тогда
		НаименованиеПолное = текНоменклатура.НаименованиеПолное;
	...
	Если СчетаУчетаНоменклатуры = Неопределено Тогда
			СчетаУчетаНоменклатуры = БухгалтерскийУчетПереопределяемый.ПолучитьСчетаУчетаНоменклатуры(Организация, текНоменклатура);
	...
	Иначе
		...
		// При отражении пени отдельно от начислений выполним подмену счетов доходов и расходов из РС "Счета учета расчетов по пени".
		// Выполнять подмену будем только если счета заполнены.
		Если УстановитьСчетаУчетаДляПени Тогда
			
			Если Не СчетаУчетаПени.СчетУчетаДоходов.Пустая() Тогда
				НоваяСтрока.СчетДоходов  = СчетаУчетаПени.СчетУчетаДоходов;
				НоваяСтрока.Субконто     = СчетаУчетаПени.Субконто1;
			КонецЕсли;
			
			Если Не СчетаУчетаПени.СчетУчетаРасходов.Пустая() Тогда
				НоваяСтрока.СчетРасходов = СчетаУчетаПени.СчетУчетаРасходов;
			КонецЕсли;
...
Функция НайтиСтрокиВТаблицеКонтрагенты(СтрокаТабличнойЧасти, ОтражениеПени)
	...
	Если ТипДокументаОтражения = "Акт об оказании производственных услуг" Тогда
		СтруктураОтбора = Новый Структура("Отражать, Контрагент, ДоговорКонтрагента, НоменклатурнаяГруппа, ЭтоКапРемонт," + ?(Не ОтражениеПени, " ЭтоЛьгота,", "") + " ЭтоПени, ВариантПоставкиУслуг");
		СтруктураОтбора.ВариантПоставкиУслуг = СтрокаТабличнойЧасти.ВариантПоставкиУслуг;
	Иначе
		СтруктураОтбора = Новый Структура("Отражать, Контрагент, ДоговорКонтрагента, НоменклатурнаяГруппа, ЭтоКапРемонт," + ?(Не ОтражениеПени, " ЭтоЛьгота,", "") + " ЭтоПени");
	КонецЕсли;
	...
	СтруктураОтбора.ЭтоКапРемонт         = СтрокаТабличнойЧасти.ЭтоКапРемонт;
...
Процедура ЗаполнитьТабличнуюЧастьОтражениеНачисленийСтарыйМеханизмОтражения(ОбновитьСодержаниеТабличнойЧасти = Ложь)
	...
	ТаблицаНачислений.Свернуть("Контрагент,ДоговорКонтрагента,ВариантПоставкиУслуг,НоменклатурнаяГруппа,ЭтоКапРемонт,ЭтоЛьгота,ЭтоПени", "");
...
Процедура СформироватьДокументОтраженияНачислений(СтрокаТабличнойЧасти,
	...
	Если ОтражениеПени Тогда
		...
		Если (мНастройкиУчетнойПолитикиТСЖ.СчетаУчетаДляОтраженияПени = Перечисления.УПЖКХ_СчетаУчетаДляПениПриОтраженииВРеглУчете.ОтдельныеСчета
			 И Не СтрокаТабличнойЧасти.ЭтоКапРемонт)
			 Или (мНастройкиУчетнойПолитикиТСЖ.СчетаУчетаДляОтраженияПениКапремонт = Перечисления.УПЖКХ_СчетаУчетаДляПениПриОтраженииВРеглУчете.ОтдельныеСчета
			 И СтрокаТабличнойЧасти.ЭтоКапРемонт) Тогда
	...
	Если СтрокаТабличнойЧасти.ЭтоКапРемонт
		 И мНастройкиУчетнойПолитикиТСЖ.ВариантОтраженияВзаиморасчетовПоКапремонту = Перечисления.КВП_ВидыСчетовРасчетовСКонтрагентамиПоКапремонту.ОтдельныеСчета Тогда
	...
	Если УстановитьСчетаУчетаДляПени Тогда
		...
		СчетаУчетаПени = Документы.КВП_ОтражениеНачисленийВРеглУчете.ПолучитьСтруктуруСчетаУчетаРасчетаПоПени(Организация,
							ТекДоговорКонтрагента.ВидВзаиморасчетов, ?(СтрокаТабличнойЧасти.ЭтоКапРемонт, Истина, Ложь));
	...
	Для Каждого СтрокаРасшифровки Из СтрокиРасшифровки Цикл
		...
		Иначе
			...
			Если УстановитьСтавкуБезНДСДляПени Тогда
				СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
			Иначе
				СтавкаНДС = Перечисления.СтавкиНДС.СтавкаНДС(СтрокаРасшифровки.Номенклатура.ВидСтавкиНДС, Дата);
			КонецЕсли;
			
			СтруктураПараметровСтроки.Вставить("СтавкаНДС",         СтавкаНДС);
	...
	Исключение
		...
		НайденныеСтроки = НайтиСтрокиВТаблицеКонтрагенты(СтрокаТабличнойЧасти, ОтражениеПени);
	...
	Если Не УстановитьСтавкуБезНДСДляПени
		 И Не СтрокаТабличнойЧасти.ЭтоКапРемонт Тогда
		ЗарегистрироватьСчетФактуруДляСтрокиТабличнойЧасти(СтрокаТабличнойЧасти, ЕстьДокумент, ДокументПроведен, КомментарийНачисления);
	КонецЕсли;
...
Процедура СформироватьДокументыТабличнойЧастиСтарыйМеханизмОтражения(ПравитьСуществующие = Ложь)
	...
	Если Не ВключатьВариантПоставкиУслуг Тогда
		ТаблицаДокументов.Свернуть("Контрагент, ДоговорКонтрагента, НоменклатурнаяГруппа, ЭтоКапРемонт, ЭтоЛьгота, ЭтоПени, Документ, СчетФактура, СчетУчетаРасчетов, СчетУчетаАвансов");
	...
	Для Каждого ТекСтрока Из ТаблицаДокументов Цикл
		...
		СтруктураОтбора                      = Новый Структура("Отражать, Контрагент, ДоговорКонтрагента, НоменклатурнаяГруппа, ЭтоКапРемонт, ЭтоЛьгота, ЭтоПени");
		СтруктураОтбора.Отражать             = Истина;
...
Функция ПолучитьТекстЗапросаГруппировкиДанныхКОтражениюНовыйАлгоритм()
	...
	|					И НЕ ДанныеКОтражению.ЭтоКапРемонт
	|				ИЛИ НЕ &ПениКапремонтОтдельно
	|					И ДанныеКОтражению.ЭтоКапРемонт)
	...
	|	ДанныеКОтражению.ЭтоКапРемонт КАК ЭтоКапРемонт
	...
	|	врДанныеКОтражениюСДополнительнымиПолями.ЭтоКапРемонт КАК ЭтоКапРемонт
	...
	|	врДанныеКОтражениюПереопределениеВидаКорректировки.ЭтоКапРемонт КАК ЭтоКапРемонт
	...
	|	врДанныеКОтражениюПереопределениеВидаКорректировки.ЭтоКапРемонт,
	...
	|			И врДанныеКОтражениюПереопределениеВидаКорректировки.ЭтоКапРемонт = врДанныеКОтражениюСГруппировкой.ЭтоКапРемонт
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
...
Процедура СформироватьДокументыНовыйМеханизмОтражения(ПравитьСуществующие = Ложь)
	...
	МассивПолейГруппировкиДетальныхЗаписей.Добавить("ЭтоКапРемонт");
							...
							Пока ВыборкаВидКорректировки.Следующий() Цикл
								...
								Пока ВыборкаДокументОтраженияКорректируемогоНачисленияПрошлогоПериода.Следующий() Цикл
									...
									Пока ВыборкаДокументОтражения.Следующий() Цикл
										...
										// Капитальный ремонт.
										ВыборкаЭтоКапРемонт = ВыборкаДокументОтражения.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
										Пока ВыборкаЭтоКапРемонт.Следующий() Цикл
											
											// Начисления с отрицательной суммой.
											ВыборкаНачисленияСОтрицательнойСуммой = ВыборкаЭтоКапРемонт.Выбрать(ОбходРезультатаЗапроса.ПоГруппировкам);
...
Процедура СформироватьДокументОтраженияЛьготыНовыйМеханизмОтражения(ТаблицаСгруппированныхДетальныхЗаписей, Параметры)
	...
	Если Параметры.ЭтоКапРемонт Тогда
...
Процедура СформироватьДокументКорректировкиРеализации(ТаблицаСгруппированныхДетальныхЗаписей, Параметры, ОтражениеПени = Ложь)
	...
	Если РезультатОтражения.Отражено
		 ...
		 И Не Параметры.ЭтоКапРемонт Тогда
...
Процедура СформироватьДокументОтраженияНачисленийНовыйМеханизмОтражения(ТаблицаСгруппированныхДетальныхЗаписей,
	...
	Если ОтражениеПени Тогда
		...
		Если (мНастройкиУчетнойПолитикиТСЖ.СчетаУчетаДляОтраженияПени = Перечисления.УПЖКХ_СчетаУчетаДляПениПриОтраженииВРеглУчете.ОтдельныеСчета
			 И Не Параметры.ЭтоКапРемонт)
			 Или (мНастройкиУчетнойПолитикиТСЖ.СчетаУчетаДляОтраженияПениКапремонт = Перечисления.УПЖКХ_СчетаУчетаДляПениПриОтраженииВРеглУчете.ОтдельныеСчета
			 И Параметры.ЭтоКапРемонт) Тогда
			УстановитьСчетаУчетаДляПени = Истина;
		КонецЕсли;
	...
	Если УстановитьСчетаУчетаДляПени Тогда
		...
		СчетаУчетаПени = Документы.КВП_ОтражениеНачисленийВРеглУчете.ПолучитьСтруктуруСчетаУчетаРасчетаПоПени(Организация,
							ТекДоговорКонтрагента.ВидВзаиморасчетов, ?(Параметры.ЭтоКапРемонт, Истина, Ложь));
	...
	Для Каждого ТекСтрокаДетальныхЗаписей Из ТаблицаСгруппированныхДетальныхЗаписей Цикл
		...
		Иначе
			...
			Если УстановитьСтавкуБезНДСДляПени Тогда
				СтавкаНДС = Перечисления.СтавкиНДС.БезНДС;
			Иначе
				СтавкаНДС = Перечисления.СтавкиНДС.СтавкаНДС(ТекСтрокаДетальныхЗаписей.Номенклатура.ВидСтавкиНДС, КонецМесяца(Дата));
			КонецЕсли;
...
Процедура ЗарегистрироватьСчетФактуру(Параметры, ДокументОтражения, РезультатОтражения, КомментарийНачисления)
	...
	Иначе
		...
		Исключение
			...
			ТекстСообщения = "В строке №" + ПредставлениеСтрок +  " при формировании счета-фактуры произошла ошибка: " + ОписаниеОшибки();
			УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьОбОшибке(ТекстСообщения, , , , Ложь);
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	// Проверим наличие сформированных и проведенных типовых бухгалтерских документов отражения начислений.
