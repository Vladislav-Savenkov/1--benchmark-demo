...
&НаКлиенте
// Процедура удаляет расшифровку по строке
//
Процедура УдалитьРасшифровкуСтроки(ЛицевойСчет, Услуга)
	
	Отбор = Новый Структура();
	Отбор.Вставить("ЛицевыеСчета", ЛицевойСчет);
	Отбор.Вставить("Услуга",       Услуга);
	
	СтрокиРасшифровки = Объект.РасшифровкаЛьгот.НайтиСтроки(Отбор);
...
Процедура ЛьготыПередУдалением(Элемент, Отказ)
	...
	УдалитьРасшифровкуСтроки(Элемент.ТекущиеДанные.ЛицевыеСчета, Элемент.ТекущиеДанные.Услуга);
	
КонецПроцедуры

&НаКлиенте
// Обработчик события "ПриНачалеРедактирования" поля "Льготы".
Процедура ЛьготыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НЕ Копирование Тогда
		...
		СлужебныеПараметры_РассчитаныЛьготыПоСтроке = Ложь;
		
		СлужебныеПараметры_ТекущийЛицевойСчет         = ТекДанные.ЛицевыеСчета;
		СлужебныеПараметры_ТекущаяУслуга              = ТекДанные.Услуга;
		СлужебныеПараметры_ТекущаяКвартира            = ТекДанные.Квартира;
		СлужебныеПараметры_ТекущийВладелец            = ТекДанные.Владелец;
		СлужебныеПараметры_ИдентификаторТекущейСтроки = ТекДанные.ПолучитьИдентификатор();
		
		Отбор = Новый Структура();
		Отбор.Вставить("ЛицевыеСчета", СлужебныеПараметры_ТекущийЛицевойСчет);
		Отбор.Вставить("Услуга",       СлужебныеПараметры_ТекущаяУслуга);
		
		СтрокиРасшифровки = Объект.РасшифровкаЛьгот.НайтиСтроки(Отбор);
		Если СтрокиРасшифровки.Количество() > 0 Тогда
			СлужебныеПараметры_РассчитаныЛьготыПоСтроке = Истина;
		КонецЕсли;
...
Процедура ЛьготыПередОкончаниемРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования, Отказ)
	...
	Если СлужебныеПараметры_РассчитаныЛьготыПоСтроке Тогда
		
		Если НЕ СлужебныеПараметры_ТекущийЛицевойСчет = ТекЛицевойСчет
			ИЛИ НЕ СлужебныеПараметры_ТекущаяУслуга = ТекУслуга Тогда
			
			ТекстВопроса = "По данной строке рассчитаны льготы. При изменении данных, льготы будут очищены!
			               |Продолжить изменение?";
			
			ПоказатьВопрос(Новый ОписаниеОповещения("ОбработатьИзменениеСтрокиЛьгот", ЭтотОбъект),
...
&НаКлиенте
// Обработчик изменения строки льгот.
//
Процедура ОбработатьИзменениеСтрокиЛьгот(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	ТабЛьгот = Объект.Льготы.НайтиПоИдентификатору(СлужебныеПараметры_ИдентификаторТекущейСтроки);
	
	Если РезультатВопроса = КодВозвратаДиалога.Нет Тогда
		
		ТабЛьгот.ЛицевыеСчета = СлужебныеПараметры_ТекущийЛицевойСчет;
		ТабЛьгот.Услуга       = СлужебныеПараметры_ТекущаяУслуга;
		ТабЛьгот.Квартира     = СлужебныеПараметры_ТекущаяКвартира;
		ТабЛьгот.Владелец     = СлужебныеПараметры_ТекущийВладелец;
		
	Иначе
		
		ТабЛьгот.Начислено = 0;
		УдалитьРасшифровкуСтроки(СлужебныеПараметры_ТекущийЛицевойСчет, СлужебныеПараметры_ТекущаяУслуга);
