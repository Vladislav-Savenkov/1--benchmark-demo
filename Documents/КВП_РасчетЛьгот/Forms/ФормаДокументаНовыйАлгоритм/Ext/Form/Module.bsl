...
Процедура ЗаполнитьДеревоЛьготПоТабличнойЧасти(СортироватьПоНомеруСтрокиВТЧ = Ложь)
	...
	Для Каждого ТекЗначение Из ТаблицаПоТЧ Цикл
		...
		Если ПервыйОбходРезультата Тогда
			...
			ВеткаПериод.ВидТарифа        = ТекЗначение.ВидТарифа;
			...
			ВеткаПериод.ВидТарифаЛьгота  = ТекЗначение.ВидТарифаЛьгота;
	...
	ЗаполнитьСписокУслугПоЛьготномуТарифу();
				...
				Если ТипЗнч(ВеткаПодУслуги.ЛицевыеСчета) = Тип("СправочникСсылка.КВП_Услуги") Тогда
					...
					Для Каждого ВеткаЛьготника Из ВеткаПодУслуги.ПолучитьЭлементы() Цикл
						...
						Для Каждого ВеткаДеталей Из ВеткаЛьготника.ПолучитьЭлементы() Цикл
							...
							НовСтр.ВидТарифа         = ВеткаДеталей.ВидТарифа;
							...
							НовСтр.ВидТарифаЛьгота   = ВеткаДеталей.ВидТарифаЛьгота;
...
&НаСервере
// Заполняет список значений ссылками на услуги из регистра сведений
// КВП_УслугиДляРасчетаЛьготПоДругимТарифам
Процедура ЗаполнитьСписокУслугПоЛьготномуТарифу()
	...
	СписокУслугПоЛьготномуТарифу.Очистить();
	...
	Если ЛьготныйТариф Тогда
		...
		Запрос.Текст = 
			"ВЫБРАТЬ
			|	КВП_УслугиДляРасчетаЛьготПоДругимТарифам.Услуга КАК Услуга
			|ИЗ
			|	РегистрСведений.КВП_УслугиДляРасчетаЛьготПоДругимТарифам КАК КВП_УслугиДляРасчетаЛьготПоДругимТарифам
			|ГДЕ
			|	КВП_УслугиДляРасчетаЛьготПоДругимТарифам.Организация = &Организация";
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			СписокУслугПоЛьготномуТарифу.Добавить(ВыборкаДетальныеЗаписи.Услуга); 
		КонецЦикла;
