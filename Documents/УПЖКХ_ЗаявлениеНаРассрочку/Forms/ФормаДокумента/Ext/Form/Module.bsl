...
&НаКлиенте
// Процедура - обработчик события "ПриИзменении" реквизита "СрокРассрочки".
//
Процедура СрокРассрочкиПриИзменении(Элемент)
	
	Если Объект.ВидРассрочки = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыРассрочки.РассрочкаНаПлатуЗаУстановкуСчетчика")
		 И Объект.СрокРассрочки > 60 Тогда
		Объект.СрокРассрочки = 60;
		УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Срок рассрочки на оплату установки счетчика не может превышать 60 месяцев.");
	КонецЕсли;
...
&НаКлиенте
// Процедура - обработчик события "Очистка" реквизита "ВидПлатежей".
//
Процедура ВидПлатежейОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры

&НаКлиенте
// Процедура - обработчик события "Очистка" реквизита "Объект".
//
...
&НаКлиенте
// Процедура - обработчик события "Очистка" реквизита "Организация".
//
...
Процедура ВидРассрочкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Не ВыбранноеЗначение = ПредыдущееЗначениеВидаРассрочки Тогда
		...
		Если ЗначениеЗаполнено(Объект.Объект) 
			...
			ТекстВопроса       = "При изменении вида рассрочки объект и таблицы рассрочки будут очищены, а срок рассрочки установлен в значение по-умолчанию. Продолжить?";
...
Процедура ВидРассрочкиОбработкаВыбораЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Иначе
		Объект.ВидРассрочки = ПредыдущееЗначениеВидаРассрочки;
		Возврат;
	КонецЕсли;
...
Процедура ВидРассрочкиОбработкаВыбораПродолжение(ВыбранноеЗначение)
	...
	Объект.ВидПлатежей = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыРассрочки.ПустаяСсылка");
...
&НаКлиенте
// Процедура обработчик событя "Очистка" поля формы "ВидРассрочки".
//
Процедура ВидРассрочкиОчистка(Элемент, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
КонецПроцедуры
...
Процедура ЗаполнитьТаблицуРассрочки(Команда)
	...
	Если Объект.Проведен Тогда
		ПоказатьПредупреждение(,"Перед заполнением отмените проведение документа.");
		Возврат;
	КонецЕсли;
...
Процедура ОбновитьТаблицуРасшифровкаРассрочки()
	...
	Элементы.РасшифровкаРассрочкиМесяцНачисления.ТекстПодвала     = "";
	Элементы.РасшифровкаРассрочкиСуммаРассрочки.ТекстПодвала      = "";
	Элементы.РасшифровкаРассрочкиКомиссияЗаРассрочку.ТекстПодвала = "";
	Элементы.РасшифровкаРассрочкиИтоговыйРазмерПлаты.ТекстПодвала = "";
	
	Если Объект.ВидПлатежей.Пустая() ИЛИ Объект.СрокРассрочки < 1 Тогда
		Возврат;
	КонецЕсли;
	...
	Если Объект.ВидРассрочки = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыРассрочки.РассрочкаНаПлатуЗаУслуги") Тогда
		...
		Элементы.НадписьПояснение.Заголовок = СтрШаблон(НадписьПояснение, Объект.СрокРассрочки);
	...
	Иначе
		...
		Если Объект.СрокРассрочки > 60 Тогда
			Возврат;
		КонецЕсли;
		...
		Элементы.НадписьПояснение.Заголовок = СтрШаблон(НадписьПояснениеЗаУстановкуСчетчика, Объект.СрокРассрочки);
	...
	Если Объект.ВидПлатежей = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыПлатежейРассрочки.Дифференцированные") Тогда
		...
		СуммаРассрочки = ТекущийРазмерПлаты / Объект.СрокРассрочки;
		
		Для Счетчик = 0 По Объект.СрокРассрочки - 1 Цикл
			...
			Если Счетчик = Объект.СрокРассрочки - 1 Тогда
	...
	Иначе
		...
		Для Счетчик = 0 По Объект.СрокРассрочки - 1 Цикл
			...
			ЗнаменательДроби = Pow(1 + МесячнаяПроцентнаяСтавка, Объект.СрокРассрочки) - 1;
...
Процедура ЗаполнитьТаблицуРассрочкиЗаУстановкуСчетчикаПродолжение()
	...
	Если Объект.Дата = '00010101' Тогда
		УПЖКХ_ТиповыеМетодыКлиентСервер.СообщитьПользователю("Не заполнена дата документа!");
		Возврат;
	КонецЕсли;
...
Процедура УправлениеФормой()
	...
	ИначеЕсли Объект.ВидРассрочки = ПредопределенноеЗначение("Перечисление.УПЖКХ_ВидыРассрочки.РассрочкаНаПлатуЗаУслуги") Тогда
		// Таблицы рассрочки
	...
	Иначе
		Элементы.НадписьПояснение.Заголовок = СтрШаблон(НадписьПояснение, "...");
	КонецЕсли;
