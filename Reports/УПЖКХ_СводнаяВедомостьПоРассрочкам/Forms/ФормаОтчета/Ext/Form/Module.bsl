...
Функция ПолучитьСведенияОРассрочках(ПараметрыОтчета)
	...
	|	УПЖКХ_СведенияОРассрочкеСрезПоследних.СрокРассрочки КАК СрокРассрочки,
	...
	|ИЗ
	|	РегистрСведений.УПЖКХ_СведенияОРассрочке.СрезПоследних(&ДатаКон, ЗаявлениеНаРассрочку.Организация = &Организация И Услуга В (&Услуги)) КАК УПЖКХ_СведенияОРассрочкеСрезПоследних
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Для каждого ТекСтрока из ТаблицаРассрочек Цикл
		...
		ТекСтрока.НепогашенныеПроцентыНаНачалоМесяца = ПолучитьСуммыПроцентовПолные(ТекСтрока.ВидПлатежей,
		                                                                            ТекСтрока.ПроцентнаяСтавка,
		                                                                            ТекСтрока.СуммаОснование,
		                                                                            ТекСтрока.НепогашенныйОстатокНаНачалоМесяца,
		                                                                            ТекСтрока.СрокРассрочки);
		
		ТекСтрока.ОстатокНаКонецМесяцаПроценты = ПолучитьСуммыПроцентовПолные(ТекСтрока.ВидПлатежей,
		                                                                      ТекСтрока.ПроцентнаяСтавка,
		                                                                      ТекСтрока.СуммаОснование,
		                                                                      ТекСтрока.ОстатокНаКонецМесяцаОсновнаяСумма,
		                                                                      ТекСтрока.СрокРассрочки);
...
Функция ПолучитьСуммыПроцентовПолные(ВидПлатежей, ПроцентнаяСтавка, СуммаОснование, Знач ОстатокСуммы, КоличествоМесяцевРассрочки)
	
	Если ОстатокСуммы <= 0 ИЛИ КоличествоМесяцевРассрочки = 0 Тогда
		
		Возврат 0;
		
	Иначе
		
		МесячнаяПроцентнаяСтавка = ПроцентнаяСтавка / 12;
		...
		Иначе
			...
			Для Счетчик = 0 По КоличествоМесяцевРассрочки - 1 Цикл
				...
				СуммаРассрочкиИтог = СуммаРассрочкиИтог + СуммаРассрочки;
